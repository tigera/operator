...
BUILD_IMAGE?=rez1ramez/operator
...
sub-non-manifest-%:
	docker run -t --entrypoint /bin/sh -v $(HOME)/.docker/config.json:/root/.docker/config.json $(CALICO_BUILD) -c "/usr/bin/manifest-tool push from-args --platforms $(call join_platforms,$(VALIDARCHES)) --template $(call unescapefs,$*$(BUILD_IMAGE):$(IMAGETAG))-amd64 --target $(call unescapefs,$*$(BUILD_IMAGE):$(IMAGETAG))"
