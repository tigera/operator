...
check-milestone: hack/bin/gh var-require-all-VERSION-GITHUB_TOKEN
	...
	$(eval MILESTONE := $(shell gh milestone list --query $(VERSION) --repo $(REPO) --state all --json title --jq '.[0].title'))
	$(if $(MILESTONE),,$(error Milestone $(VERSION) does not exist))
	@echo "Checking $(VERSION) milestone has no open PRs"
	$(eval OPEN_PRS := $(shell gh search prs --milestone $(VERSION) --repo $(REPO) --state open --json number --jq '.[0].number'))
