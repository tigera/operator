...
type InstallationSpec struct {
	...
	// CertificateManagement configures pods to submit a CertificateSigningRequest to API certificates.k8s.io/v1beta1 in order
...
type CertificateManagement struct {
	// Root CA certificate of the certificate authority that signs the certificate requests in PEM format.
	RootCA string `json:"rootCA"`

	// When a CSR is issued to API certificates.k8s.io the signerName is added to the request in order to accommodate for clusters
	// with multiple signers.
	// Must be formatted as: "<my-domain>/<my-signername>".
	SignerName string `json:"signerName"`

	// Specify the algorithm used by pods to generate a key pair as part of the X.509 certificate request.
