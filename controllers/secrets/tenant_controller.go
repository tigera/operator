...
func (r *TenantController) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// Get the Tigera CA certificate.
	tigeraCA, err := cm.GetCertificate(r.client, certificatemanagement.CASecretName, common.OperatorNamespace())
	if err != nil {
	...
	// TODO: We may need two bundles - one with public certs and one without.
	trustedBundle := cm.CreateTrustedBundle()
	trustedBundle.AddCertificates(tigeraCA)
