...
func refExistsInRemote(lsRemoteOutput, ref string) bool {
	for _, line := range strings.Split(lsRemoteOutput, "\n") {
		...
		remoteRef := parts[1]
		// Extract the ref name from the full path
		name := remoteRef
		if idx := strings.LastIndex(remoteRef, "/"); idx != -1 {
			name = remoteRef[idx+1:]
		}
		if name == ref {
			return true
		}
...
var validateBranchRefs = func(ctx context.Context, c *cli.Command) (context.Context, error) {
	...
	if valid, err := isValidStream(stream); err != nil {
		...
		return ctx, fmt.Errorf("stream %q is not valid, expected format %s", stream, streamFormat)
	}
