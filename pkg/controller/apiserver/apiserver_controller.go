...
func (r *ReconcileAPIServer) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if installationSpec.Variant == operatorv1.TigeraSecureEnterprise {
		...
		// it can sign certificates for managed clusters. However, if the manager_controller has not yet created the
		// tunnel secret, we should not degrade. This is because the manager_controller exits the reconcile loop if the
