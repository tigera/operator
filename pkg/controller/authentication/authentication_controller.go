...
func (r *ReconcileAuthentication) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	preDefaulted := authentication.DeepCopy()
	...
	patch, err := utils.CreatePatch(preDefaulted, authentication)
	if err != nil {
		log.Error(err, "Failed to create patch to write defaults")
		r.status.SetDegraded("Failed to create patch to write defaults", err.Error())
		return reconcile.Result{}, err
	}
	if err = r.client.Patch(ctx, authentication, patch); err != nil {
