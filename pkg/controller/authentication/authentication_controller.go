...
func add(mgr manager.Manager, c ctrlruntime.Controller) error {
	...
	if err = utils.AddSecretProviderClassWatch(c, render.OIDCSecretProviderClassName, render.DexNamespace); err != nil {
		return fmt.Errorf("%s failed to watch the secret provider class '%s' in '%s' namespace: %w", controllerName, render.OIDCSecretProviderClassName, render.DexNamespace, err)
	}
