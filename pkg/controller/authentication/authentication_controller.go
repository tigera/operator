...
func add(mgr manager.Manager, r *ReconcileAuthentication) error {
	...
	for _, namespace := range []string{rutil.OperatorNamespace(), rdex.Namespace} {
...
func (r *ReconcileAuthentication) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.Get(ctx, types.NamespacedName{Name: rdex.TLSSecretName, Namespace: rutil.OperatorNamespace()}, tlsSecret); err != nil {
		if errors.IsNotFound(err) {
	...
	idpSecret = rutil.CopySecrets(rdex.Namespace, idpSecret)[0]
