...
func (r *ReconcileAuthentication) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// TODO <before PR is closed> figure out a better way to handle the authentication resource not being "available"
	// (since dex is removed so there are no resources to watch).
	if !disableDex {
