...
func add(mgr manager.Manager, r *ReconcileAuthentication) error {
	...
	for _, namespace := range []string{rcommon.OperatorNamespace(), render.DexNamespace} {
...
func (r *ReconcileAuthentication) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.Get(ctx, types.NamespacedName{Name: render.DexTLSSecretName, Namespace: rcommon.OperatorNamespace()}, tlsSecret); err != nil {
		if errors.IsNotFound(err) {
	...
	idpSecret = rcommon.CopySecrets(render.DexNamespace, idpSecret)[0]
