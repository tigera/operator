...
// Add creates a new BgpConfiguration Controller and adds it to the Manager. The Manager will set fields on the Controller
...
// ReconcileBgpConfiguration reconciles a BgpConfiguration object
...
// Reconcile reads that state of the cluster for a BgpConfiguration object and makes changes based on the state read
// and what is in the BgpConfiguration.Spec
// Note:
...
func (r *ReconcileBgpConfiguration) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	bgpConfig := &v1.BGPConfiguration{}
	err := r.client.Get(ctx, utils.DefaultTSEEInstanceKey, bgpConfig)
	if err != nil {
		if errors.IsNotFound(err) {
			//reqLogger.V(5).Info("BgpConfiguration CR not found", "err", err)
			reqLogger.Info("BgpConfiguration CR not found", "err", err)
			r.status.OnCRNotFound()
			return reconcile.Result{}, err
		}
		reqLogger.Error(err, "Error querying BgpConfiguration")
		r.status.SetDegraded("Error querying BgpConfiguration", err.Error())
		return reconcile.Result{}, err
	}

	r.status.OnCRFound()
	//reqLogger.V(2).Info("Loaded config", "config", bgpConfig)
	reqLogger.Info("Loaded config", "config", bgpConfig)
	preDefaultPatchFrom := client.MergeFrom(bgpConfig.DeepCopy())

	// Write the discovered configuration back to the API. This is essentially a poor-man's defaulting, and
	// ensures that we don't surprise anyone by changing defaults in a future version of the operator.
	if err = r.client.Patch(ctx, bgpConfig, preDefaultPatchFrom); err != nil {
