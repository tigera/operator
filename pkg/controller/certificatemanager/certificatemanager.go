...
func (cm *certificateManager) getKeyPair(cli client.Client, secretName, secretNamespace string, readCertOnly bool) (certificatemanagement.KeyPairInterface, *x509.Certificate, error) {
	...
	if x509Cert.Issuer.CommonName == rmeta.TigeraOperatorCAIssuerPrefix {
		// A new-style certificate issued by tigera-operator.
		if cm.keyPair.CertificateManagement != nil {
			return certificateManagementKeyPair(cm, secretName, nil), nil, nil
		}
	...
	} else if strings.HasPrefix(x509Cert.Issuer.CommonName, rmeta.TigeraOperatorCAIssuerPrefix) {
		// This is a legacy-style certificate generated by an older version of the operator.
		// Regenerate it if needed, otherwise we'll just continue to use it.
		if !readCertOnly && !ValidForClientAndServer(x509Cert) {
		...
		// This is a BYO certificate not originally generated by the operator.
		if !readCertOnly && !ValidForClientAndServer(x509Cert) {
			// It isn't valid for use as a client and server certificate. We need to
