...
func (r *ReconcileConnection) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := utils.ApplyDefaults(ctx, r.cli, managementClusterConnection, func() {
		if instl.Variant == operatorv1.Calico {
			// // There is no impersonation in OSS by design. We will remove it here, rather than degrading. We want
			// to avoid degrading a cluster for having a value that may have previously been defaulted by the operator.
			managementClusterConnection.Spec.Impersonation = nil
		} else {
			var wildcard = []string{"*"}
