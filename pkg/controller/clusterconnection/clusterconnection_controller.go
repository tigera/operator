...
func (r *ReconcileConnection) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	certificateManager, err := certificatemanager.Create(r.cli, instl, r.clusterDomain, common.OperatorNamespace())
	if err != nil {
		r.status.SetDegraded(operatorv1.ResourceCreateError, "Unable to create the certificate manager", err, reqLogger)
		return reconcile.Result{}, err
	}

	trustedCertBundle, err := certificateManager.LoadTrustedBundle(ctx, r.cli, render.GuardianNamespace)
	if err != nil {
	...
	//s, err := utils.GetSecret(ctx, r.cli, render.PacketCaptureServerCert, common.OperatorNamespace())
	//if err != nil {
	...
	//if err := r.cli.Get(ctx, utils.DefaultTSEEInstanceKey, monitorCR); err != nil {
