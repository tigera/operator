...
var _ = Describe("Compliance controller tests", func() {
	...
	BeforeEach(func() {
		...
		Expect(c.Create(ctx, &corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: esgateway.EsGatewayElasticPublicCertSecret, Namespace: "tigera-operator"}})).NotTo(HaveOccurred())
	...
	It("should create a new compliance server cert if the cert is owned by compliance and has the wrong DNS names", func() {
		...
		newSecret, err := secret.CreateTLSSecret(nil,
			render.ComplianceServerCertSecret, rmeta.OperatorNamespace(), corev1.TLSPrivateKeyKey,
