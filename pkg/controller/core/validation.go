...
func validateCustomResource(instance *operatorv1alpha1.Core) error {
	if instance.Spec.RunKubeProxy {
		if len(instance.Spec.APIServer) == 0 {
			return fmt.Errorf("spec.apiServer required for kubeProxy installation")
		} else if _, err := url.ParseRequestURI(instance.Spec.APIServer); err != nil {
			return fmt.Errorf("spec.apiServer contains invalid ")
		}
	}
	return nil
}
