...
var doNotReconcileUntil *time.Time

// allowSetupOfAWSSecurityGroups returns true if it is ok to attempt to configure the
// AWS SecurityGroups. This is to prevent spamming the AWS API and hitting the rate limits.
func allowSetupOfAWSSecurityGroups() (bool, time.Duration) {
	if doNotReconcileUntil == nil {
		return true, 0
	}
	now := time.Now()
	if doNotReconcileUntil.After(now) {
		return true, 0
	}
	return false, doNotReconcileUntil.Sub(now)
}
...
func getAWSCreds(ctx context.Context, client client.Client) (id, secret string, err error) {
	...
	log.V(traceLevel).Info("Retrieved aws-creds", "aws-creds", creds)
...
func getAvailabilityZone() (string, error) {
	...
	log.V(traceLevel).Info("Retrieved availability zone", "availability-zone", az)
	return az, nil
}
...
func getAWSRegion() (string, error) {
	...
	log.V(traceLevel).Info("Parsed region", "region", m[1])
	return m[1], nil
}
...
func getVPCid() (string, error) {
	...
	log.V(traceLevel).Info("MACs read from metadata", "MACs", macs)
	if len(macs) < 1 {
	...
	log.V(traceLevel).Info("VPC id read from metadata", "VPCid", vpcId)
	return vpcId, nil
}
...
func getSGGroup(cli *ec2.EC2, vpcId string, nameFilter string) (*ec2.SecurityGroup, error) {
	...
	log.V(traceLevel).Info("DescribeSecurityGroups", "SecurityGroupOutput", out)
...
func allowIngressToSG(cli *ec2.EC2, toSG *ec2.SecurityGroup, sources []ingressSrc) error {
	...
	in.SetGroupId(aws.StringValue(toSG.GroupId))
	for _, s := range sources {
		log.V(traceLevel).Info("Ingress src being added", "ingressSrc", s.String())
		skip := false
		for _, x := range toSG.IpPermissions {
			...
			for _, y := range x.UserIdGroupPairs {
				if *y.GroupId == s.srcSGId {
					...
					// ingress rule.
					log.V(traceLevel).Info("Ingress rule already exists", "toSG", toSG, "ingressSrc", s.String())
					skip = true
					break
				}
			}
		...
		log.V(traceLevel).Info("Added Ingress rule", "toSG", toSG, "ingressSrc", s.String())
	}
	return nil
}
