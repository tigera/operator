...
func newReconciler(mgr manager.Manager, opts options.ControllerOptions) (*ReconcileInstallation, error) {
	...
	r := &ReconcileInstallation{
		...
		fieldManager:        managedfields.New(mgr.GetClient()),
		opts:                opts,
...
func (r *ReconcileInstallation) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareFelixConfiguration(ctx, instance, needsNamespaceMigration)); err != nil {
	...
	felixConfiguration, err := managedfields.Apply(ctx, r.fieldManager, r.declareBPFEnabled(ctx, instance, needsNamespaceMigration))
	if err != nil {
		if !managedfields.IsFieldConflict(err) {
			r.status.SetDegraded(operatorv1.ResourceUpdateError, "Error updating FelixConfiguration", err, reqLogger)
			return reconcile.Result{}, err
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareBGPConfiguration(instance)); err != nil {
		if !managedfields.IsFieldConflict(err) {
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareBPFEnabled(ctx, instance, needsNamespaceMigration)); err != nil {
		if !managedfields.IsFieldConflict(err) {
...
func allNodesRunTargetVersion(install *operatorv1.Installation, needNsMigration, nodeDSExists bool, targetVersion string) bool {
	...
	// Written together, only once the stack was seen available on that version. A status
	// reporting neither also fails.
