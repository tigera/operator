...
func (r *ReconcileInstallation) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if r.enterpriseCRDsExist {
		...
		elasticsearchSecret, err = utils.GetSecret(ctx, r.client, relasticsearch.EsGatewayPublicCertSecret, rmeta.OperatorNamespace())
		if err != nil {
			log.Error(err, err.Error())
		...
		kibanaSecret, err = utils.GetSecret(ctx, r.client, render.EsGatewayKibanaPublicCertSecret, rmeta.OperatorNamespace())
		if err != nil {
			log.Error(err, err.Error())
