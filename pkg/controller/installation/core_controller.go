...
func (r *ReconcileInstallation) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// Seeded here, right after the namespace it lives in, so the switch exists even on
	// a cluster that is degraded for an unrelated reason.
	if instance.Spec.Variant.IsEnterprise() {
...
// not already present. This controller owns it because every cluster the feature can be
// enabled on needs one: a managed cluster has no Manager CR, but rbacsync still reads
// that cluster's own copy over the tunnel.
//
// Create-only, so an admin's toggle is never overwritten. No OwnerReference (nil CR), so
// the toggle survives Installation deletion.
//
// Deleting the ConfigMap is fail-closed: ui-apis and rbacsync both read a missing
// ConfigMap as disabled, and this re-seeds it disabled — the operator does not track the
// previous value. Logged because the feature going quiet is otherwise invisible.
