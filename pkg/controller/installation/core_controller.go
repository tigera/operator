...
func (r *ReconcileInstallation) setDefaultsOnFelixConfiguration(ctx context.Context, install *operatorv1.Installation, fc *v3.FelixConfiguration, reqLogger logr.Logger, needNsMigration bool) (bool, error) {
	...
	// A running calico-node DaemonSet decides both what version is out there and how
	// bpfEnabled is derived, below.
	ds := &appsv1.DaemonSet{}
	nodeDSExists := true
	...
	if allNodesRunTargetVersion(install, needNsMigration, nodeDSExists, productVersion(&install.Spec)) &&
...
// productVersion is the release the operator ships for this variant.
func productVersion(install *operatorv1.InstallationSpec) string {
