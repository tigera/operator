...
func (r *ReconcileInstallation) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// If we are switching from this mode to EKS networking, we need to restart the operator to enable the AmazonCloudIntegration controller.
	if instance.Spec.KubernetesProvider == operator.ProviderEKS && instance.Spec.Variant == operator.TigeraSecureEnterprise {
		...
		// Queue a retry. We don't want to watch the AmazonCloudIntegration API since it might not exist and would cause
		// this controller to fail.
		reqLogger.Info("Scheduling a retry in 30 seconds")
		return reconcile.Result{RequeueAfter: 30 * time.Second}, nil
	}
