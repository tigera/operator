...
func (r *ReconcileInstallation) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.Spec.KubernetesProvider == operator.ProviderOpenShift {
		...
		reqLogger.WithValues("OpenshiftNetworkConfig", openshiftConfig).V(1).Info("Updating OpenShift cluster network status")
		...
		if err = r.client.Patch(ctx, openshiftConfig, patchFrom); err != nil {
			r.SetDegraded("Error patching openshift network status", err, reqLogger.WithValues("OpenshiftNetworkConfig", openshiftConfig))
			return reconcile.Result{}, err
		}
	}
