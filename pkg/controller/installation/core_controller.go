...
func (r *ReconcileInstallation) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if installationMarkedForDeletion {
		...
		if err != nil && !apierrors.IsNotFound(err) {
			r.status.SetDegraded(operator.ResourceReadError, "Unable to read CSI daemonset", err, reqLogger)
			return reconcile.Result{}, err
		} else if apierrors.IsNotFound(err) {
			...
			if err != nil {
				r.status.SetDegraded(operator.ResourceReadError, "Unable to read CSI daemonset pods", err, reqLogger)
				return reconcile.Result{}, err
			}
			if !terminated {
