...
var _ = Describe("Testing core-controller installation", func() {
	...
	Context("Reconcile tests", func() {
		...
		It("should set BPFKubeProxyHealthzPort once calico-node reports the target version", func() {
			...
			cr.Status.Variant = cr.Spec.Variant
			cr.Status.CalicoVersion = r.ext.ProductVersion(&cr.Spec)
		...
		It("should not set BPFKubeProxyHealthzPort while the kube-system migration is pending", func() {
			// Manifest-install calico-node pods still serve nodes from kube-system, so the
			// operator's DaemonSet says nothing about them.
