...
func validateCustomResource(instance *operatorv1.Installation) error {
	...
	if v4pool := GetIPv4Pool(instance); v4pool != nil {
		...
		if !valid {
			return fmt.Errorf("%s is invalid for ipPool.encapsulation, should be one of %s", v4pool.Encapsulation,
		...
		if !valid {
			return fmt.Errorf("%s is invalid for ipPool.natOutgoing, should be one of %s", v4pool.NATOutgoing,
