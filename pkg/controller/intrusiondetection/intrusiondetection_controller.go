...
func add(mgr manager.Manager, c controller.Controller) error {
	...
	for _, secretName := range []string{
		render.EsGatewayElasticPublicCertSecret, render.ElasticsearchIntrusionDetectionUserSecret,
		render.ElasticsearchIntrusionDetectionJobUserSecret, render.ElasticsearchADJobUserSecret,
		render.EsGatewayKibanaPublicCertSecret, render.ManagerInternalTLSSecretName,
	} {
	...
	if err = utils.AddSecretsWatch(c, render.EsGatewayElasticPublicCertSecret, render.IntrusionDetectionNamespace); err != nil {
		return fmt.Errorf("intrusiondetection-controller failed to watch the Secret resource: %v", err)
	}

	if err = utils.AddSecretsWatch(c, render.EsGatewayKibanaPublicCertSecret, render.IntrusionDetectionNamespace); err != nil {
		return fmt.Errorf("intrusiondetection-controller failed to watch the Secret resource: %v", err)
	}
