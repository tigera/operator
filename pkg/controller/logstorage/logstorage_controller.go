...
func (r *ReconcileLogStorage) getElasticNativeUserConfig(ctx context.Context, dexCfg render.DexRelyingPartyConfig, licenseType render.ElasticLicenseType) render.ElasticNativeUsersConfig {
	...
	objs := &[]runtime.Object{
		...
		&corev1.Secret{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:      render.EsNativeUsersSecreteName,
				Namespace: render.ElasticsearchNamespace,
			},
		},
	}

	cmErr := r.client.Get(ctx, types.NamespacedName{Name: render.EsNativeUsersConfigMapName, Namespace: render.ElasticsearchNamespace}, &corev1.ConfigMap{})
	secretErr := r.client.Get(ctx, types.NamespacedName{Name: render.EsNativeUsersSecreteName, Namespace: render.ElasticsearchNamespace}, &corev1.Secret{})
