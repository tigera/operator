...
func (r *ReconcileLogStorage) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if managementClusterConnection == nil {
		...
		kubeControllersGatewaySecret, err := utils.GetSecret(ctx, r.client, render.ElasticsearchKubeControllersUserSecret, common.CalicoNamespace)
		if err != nil {
			return reconcile.Result{}, err
		}
		if kubeControllersGatewaySecret == nil {
			r.status.SetDegraded("Waiting for kube-controllers gateway user secret to be available", "")
			return reconcile.Result{}, nil
		}

		kubeControllersVerificationSecret, kubeControllersSecureUserSecret, err := r.createKubeControllersSecrets(ctx, kubeControllersGatewaySecret, esAdminUserSecret)
		if err != nil {
			reqLogger.Error(err, err.Error())
			r.status.SetDegraded("Failed to create kube-controllers secrets for ES gateway", "")
			return reconcile.Result{}, err
		}
		...
		esGatewayComponent := esgateway.EsGateway(
			...
			[]*corev1.Secret{kubeControllersVerificationSecret, kubeControllersSecureUserSecret},
...
func (r *ReconcileLogStorage) createKubeControllersSecrets(ctx context.Context, kubeControllersGatewaySecret *corev1.Secret, esAdminUserSecret *corev1.Secret) (*corev1.Secret, *corev1.Secret, error) {
	...
	return kubeControllersVerificationSecret, kubeControllersSecureUserSecret, nil
}
...
// 1) The certificate secret needed for ES Gateway (in the operator namespace). If the user didn't create this it is created.
