...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	for _, secretName := range []string{
		...
		render.OIDCSecretName, render.DexObjectName, render.EsGatewayElasticPublicCertSecret,
		render.EsGatewayKibanaPublicCertSecret, render.EsGatewayTLSSecret} {
	...
	if err = utils.AddSecretsWatch(c, render.EsGatewayTLSSecret, render.ElasticsearchNamespace); err != nil {
		return fmt.Errorf("log-storage-controller failed to watch the Secret resource: %w", err)
	}

	if err = utils.AddSecretsWatch(c, render.EsGatewayElasticPublicCertSecret, render.ElasticsearchNamespace); err != nil {
		return fmt.Errorf("log-storage-controller failed to watch the Secret resource: %w", err)
	}

	if err = utils.AddSecretsWatch(c, render.EsGatewayKibanaPublicCertSecret, render.KibanaNamespace); err != nil {
		return fmt.Errorf("log-storage-controller failed to watch the Secret resource: %w", err)
	}
	...
	if managementClusterConnection == nil {
		...
		if tlsSecret == nil {
			...
			tlsSecret, err = utils.EnsureCertificateSecret(
				render.EsGatewayTLSSecret, tlsSecret, render.EsGatewaySecretKeyName, render.EsGatewaySecretCertName, certDur, svcDNSNames...,
			)

			if err != nil {
				r.status.SetDegraded(fmt.Sprintf("Error ensuring ES Gateway TLS certificate %q exists and has valid DNS names", render.EsGatewayElasticPublicCertSecret), err.Error())
				return reconcile.Result{}, err
			}
		}
