...
func (r *ReconcileLogStorage) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if managementClusterConnection == nil {
		...
		result, err = r.applyILMPolicies(ls, reqLogger, ctx)
		if err != nil || result.Requeue {
			return result, err
		}

		result, err = r.validateLogStorage(curatorSecrets, esLicenseType, reqLogger, ctx)
		if err != nil || result.Requeue {
			return result, err
		}

		result, err = r.createEsMetrics(
			install,
			variant,
			pullSecrets,
			reqLogger,
