...
func (r *ReconcileLogStorage) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if managementClusterConnection == nil {
		...
		if tlsSecret == nil {
			...
			if err != nil {
				r.status.SetDegraded(fmt.Sprintf("Error ensuring ES Gateway TLS certificate %q exists and has valid DNS names", esgateway.EsGatewayElasticPublicCertSecret), err.Error())
				return reconcile.Result{}, err
			}
		}
	...
	if managementClusterConnection == nil {
		...
		esGatewayComponent := esgateway.EsGateway(ls, install, pullSecrets, esAdminUserSecret, kibanaPublicCertSecret, tlsSecret, r.clusterDomain)
