...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &corev1.Secret{}}, &handler.EnqueueRequestForObject{}, &predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			_, hasLabel := e.Object.GetLabels()[utils.TigeraElasticsearchUserSecretLabel]
			return e.Object.GetNamespace() == rmeta.OperatorNamespace() && hasLabel
		},
		UpdateFunc: func(e event.UpdateEvent) bool {
			_, hasLabel := e.ObjectNew.GetLabels()[utils.TigeraElasticsearchUserSecretLabel]
...
func (r *ReconcileLogStorage) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if managementClusterConnection == nil {
		var flowShards = utils.CalculateFlowShards(ls.Spec.Nodes, utils.DefaultElasticsearchShards)
		clusterConfig = relasticsearch.NewClusterConfig(render.DefaultElasticsearchClusterName, ls.Replicas(), utils.DefaultElasticsearchShards, flowShards)
		...
		if gatewayCertSecret, publicCertSecret, customerProvidedCert, err = utils.GetESGatewayCertificateSecrets(ctx, install, r.client, r.clusterDomain); err != nil {
			reqLogger.Error(err, err.Error())
	...
	if managementClusterConnection == nil {
		...
		kubeControllersGatewaySecret, kubeControllersVerificationSecret, kubeControllersSecureUserSecret, err := utils.CreateKubeControllersSecrets(ctx, esAdminUserSecret, r.client)
		if err != nil {
			reqLogger.Error(err, err.Error())
	...
	if instl.CertificateManagement != nil {
		...
		if internalSecret != nil {
			...
			if err == utils.ErrInvalidCertDNSNames {
				if err := utils.DeleteInvalidECKManagedPublicCertSecret(ctx, internalSecret, r.client); err != nil {
					return nil, nil, err
				}
			}
		} else {
