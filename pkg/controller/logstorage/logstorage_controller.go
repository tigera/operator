...
type keyPairCollection struct {
	...
	esProxy            certificatemanagement.CertificateInterface
	fluentd            certificatemanagement.CertificateInterface
	intrusionDetection certificatemanagement.CertificateInterface
...
func (r *ReconcileLogStorage) generateSecrets(
	...
	esProxyCert, err := cm.GetCertificate(r.client, render.ManagerTLSSecretName, common.OperatorNamespace())
	if err != nil {
		r.status.SetDegraded(operatorv1.ResourceReadError, "Failed to get certificate", err, log)
		return nil, err
	} else if esProxyCert == nil {
		// ES proxy is deployed after the first execution of this controller, so it is normal for the
		// secret to not exist. We'll add it to the trusted bundle when it becomes avaialble though.
		log.Info("es-proxy cert not available yet, will add it to the bundle when it becomes available")
	} else {
		collection.esProxy = esProxyCert
	}

	// Get certificate for fluentd, which Linseed needs to trust in a standalone or management cluster.
	fluentdCert, err := cm.GetCertificate(r.client, render.FluentdPrometheusTLSSecretName, common.OperatorNamespace())
	if err != nil {
		r.status.SetDegraded(operatorv1.ResourceReadError, "Failed to get certificate", err, log)
		return nil, err
	} else if fluentdCert == nil {
		// Fluentd is deployed after the first execution of this controller, so it is normal for the
		// secret to not exist. We'll add it to the trusted bundle when it becomes avaialble though.
		log.Info("fluentd cert not available yet, will add it to the bundle when it becomes available")
	} else {
		collection.fluentd = fluentdCert
	}

	// Get certificate for intrusion detection controller, which Linseed needs to trust in a standalone or management cluster.
	intrusionDetectionCert, err := cm.GetCertificate(r.client, render.IntrusionDetectionTLSSecretName, common.OperatorNamespace())
	if err != nil {
		r.status.SetDegraded(operatorv1.ResourceReadError, "Failed to get certificate", err, log)
		return nil, err
	} else if intrusionDetectionCert == nil {
		// Intrusion Detection is deployed after the first execution of this controller, so it is normal for the
		// secret to not exist. We'll add it to the trusted bundle when it becomes avaialble though.
		log.Info("intrusion detection cert not available yet, will add it to the bundle when it becomes available")
	} else {
		collection.intrusionDetection = intrusionDetectionCert
	}
