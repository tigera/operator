...
var _ = Describe("Log storage controller", func() {
	...
	Context("Managed Cluster", func() {
		...
		It("tests that the ExternalService is setup", func() {
			...
			Expect(svc.Spec.ExternalName).Should(Equal("tigera-guardian.tigera-mcm.svc.othername.local"))
		...
		It("tests an error is returned if the LogStorage resource exists", func() {
			...
			Expect(err.Error()).Should(Equal("cluster type managed but logstorage exists"))
		})
	})
