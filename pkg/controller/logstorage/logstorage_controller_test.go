...
var (
	...
	esCertSecretOperKey    = client.ObjectKey{Name: render.TigeraElasticsearchCertSecret, Namespace: rcommon.OperatorNamespace()}
	esCertPubSecretKey     = client.ObjectKey{Name: render.ElasticsearchPublicCertSecret, Namespace: render.ElasticsearchNamespace}
	esCertPubSecretOperKey = client.ObjectKey{Name: render.ElasticsearchPublicCertSecret, Namespace: rcommon.OperatorNamespace()}

	kbCertSecretKey        = client.ObjectKey{Name: render.TigeraKibanaCertSecret, Namespace: render.KibanaNamespace}
	kbCertSecretOperKey    = client.ObjectKey{Name: render.TigeraKibanaCertSecret, Namespace: rcommon.OperatorNamespace()}
	kbCertPubSecretKey     = client.ObjectKey{Name: render.KibanaPublicCertSecret, Namespace: render.KibanaNamespace}
	kbCertPubSecretOperKey = client.ObjectKey{Name: render.KibanaPublicCertSecret, Namespace: rcommon.OperatorNamespace()}
	...
	curatorUsrSecretObjMeta  = metav1.ObjectMeta{Name: render.ElasticsearchCuratorUserSecret, Namespace: rcommon.OperatorNamespace()}
	operatorUsrSecretObjMeta = metav1.ObjectMeta{Name: render.ElasticsearchOperatorUserSecret, Namespace: rcommon.OperatorNamespace()}
		...
		Context("Unmanaged cluster", func() {
			Context("successful LogStorage Reconcile", func() {
				...
				It("test LogStorage reconciles successfully", func() {
					...
					Expect(cli.Get(ctx, esCertSecretKey, secret)).ShouldNot(HaveOccurred())
					esPublicSecret := createPubSecret(render.ElasticsearchPublicCertSecret, render.ElasticsearchNamespace, secret.Data["tls.crt"], "tls.crt")
				...
				It("test that LogStorage is degraded if the user-supplied certs have invalid DNS names", func() {
					...
					esSecret, err := rcommon.CreateOperatorTLSSecret(testCA,
						render.TigeraElasticsearchCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, "tigera-secure-es-http.tigera-elasticsearch.svc",
					)
					Expect(err).ShouldNot(HaveOccurred())
					...
					kbSecret, err := rcommon.CreateOperatorTLSSecret(testCA,
						render.TigeraKibanaCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, "tigera-secure-kb-http.tigera-elasticsearch.svc",
					)
					Expect(err).ShouldNot(HaveOccurred())
				...
				It("test that LogStorage reconciles if the user-supplied certs have the expected DNS names", func() {
					...
					testCA := test.MakeTestCA("logstorage-test")
					esSecret, err := rcommon.CreateOperatorTLSSecret(testCA,
						render.TigeraElasticsearchCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, dnsNames...,
					)
					Expect(err).ShouldNot(HaveOccurred())
					...
					kbSecret, err := rcommon.CreateOperatorTLSSecret(testCA,
						render.TigeraKibanaCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, dnsNames...,
					)
					Expect(err).ShouldNot(HaveOccurred())
					kbPublicSecret := createPubSecret(render.KibanaPublicCertSecret, render.KibanaNamespace, kbSecret.Data["tls.crt"], "tls.crt")
					Expect(cli.Create(ctx, kbSecret)).ShouldNot(HaveOccurred())
				...
				It("test that LogStorage creates new certs if operator managed certs have invalid DNS names", func() {
					...
					esSecret, err = rcommon.CreateOperatorTLSSecret(nil,
						render.TigeraElasticsearchCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, "tigera-secure-es-http.tigera-elasticsearch.svc",
					)
					Expect(err).ShouldNot(HaveOccurred())

					kbSecret, err = rcommon.CreateOperatorTLSSecret(nil,
						render.TigeraKibanaCertSecret, "tls.key", "tls.crt", rcommon.DefaultCertificateDuration, nil, "tigera-secure-kb-http.tigera-elasticsearch.svc",
					)
					Expect(err).ShouldNot(HaveOccurred())
					...
					Expect(cli.Create(
						ctx, createPubSecret(render.ElasticsearchPublicCertSecret, render.ElasticsearchNamespace, esSecret.Data["tls.crt"], "tls.crt"),
					)).ShouldNot(HaveOccurred())
...
func createESSecrets() []client.Object {
	...
	esOperNsSecret := rcommon.CopySecrets(render.ElasticsearchNamespace, esSecret)[0]

	esPublicOperNsSecret := createPubSecret(render.ElasticsearchPublicCertSecret, rcommon.OperatorNamespace(), esSecret.Data["tls.crt"], "tls.crt")
	esPublicSecret := createPubSecret(render.ElasticsearchPublicCertSecret, render.ElasticsearchNamespace, esSecret.Data["tls.crt"], "tls.crt")
...
func createKibanaSecrets() []client.Object {
	...
	kbOperNsSecret := rcommon.CopySecrets(render.KibanaNamespace, kbSecret)[0]

	kbPublicOperNsSecret := createPubSecret(render.KibanaPublicCertSecret, rcommon.OperatorNamespace(), kbSecret.Data["tls.crt"], "tls.crt")
	kbPublicSecret := createPubSecret(render.KibanaPublicCertSecret, render.KibanaNamespace, kbSecret.Data["tls.crt"], "tls.crt")
	return []client.Object{
		kbSecret,
		kbOperNsSecret,
