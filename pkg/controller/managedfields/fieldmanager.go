...
func Apply[U any, T object[U]](ctx context.Context, m *FieldManager, declare DeclareFn[T]) (T, error) {
	...
	return applyDeclared[U](ctx, m, current, declare)
}
...
func applyDeclared[U any, T object[U]](ctx context.Context, m *FieldManager, current T, declare DeclareFn[T]) (T, error) {
	var zero T
	...
	if err := clearLegacyOwned(ctx, m, current, gvk, d, payload); err != nil {
		return zero, err
	}
...
// does not own.
func clearLegacyOwned[T client.Object](ctx context.Context, m *FieldManager, current client.Object, gvk schema.GroupVersionKind, d *Declaration[T], payload *unstructured.Unstructured) error {
