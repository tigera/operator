...
func (r *ReconcileManager) resolveGateway(
	...
	// only the host must match spec.gateway.hostname — scheme and port are
	// ignored.
	if authenticationCR != nil && authenticationCR.Spec.ManagerDomain != "" {
		if managerDomainHost(authenticationCR.Spec.ManagerDomain) != gw.Hostname {
			err := fmt.Errorf("Authentication.spec.managerDomain %q does not match spec.gateway.hostname %q — OIDC redirects will fail",
