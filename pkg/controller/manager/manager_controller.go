...
const (
	...
	// ExternalTunnelCAAnnotation marks a tenant whose voltron tunnel CA is created outside the operator. The tunnel
	// CA doubles as voltron's tunnel server certificate, so its DNS SAN is the SNI every managed cluster in the
	// tenant dials; delivering it externally is what lets that SAN be chosen up front rather than defaulted to the
	// bare tenant ID here. For an annotated tenant the operator consumes the tunnel secret but never creates or
	// updates it, and waits if it is not there yet.
