	...
	Context("reconciliation", func() {
		...
		Context("single-tenant", func() {
			...
			Context("Multi-cluster reconciliation", func() {
				...
				Context("with an externally managed tunnel CA", func() {
					// A single-tenant cluster whose tunnel CA is delivered from outside the operator -- in Calico Cloud,
					// generated by tds and applied as a sealed secret by the cluster's argo application. It carries the
					// SAN <authID>.<cloud-domain> rather than the "voltron" the operator defaults to, which is what makes
					// the cluster's tunnel addressable by name.
					//
					// There is no Tenant on a single-tenant install, so the annotation lives on the ManagementCluster.
					const externalSAN = "abc123.dev.calicocloud.io"

					createManagementCluster := func(external bool) {
						managementCluster := &operatorv1.ManagementCluster{
							ObjectMeta: metav1.ObjectMeta{Name: "tigera-secure"},
						}
						if external {
							managementCluster.Annotations = map[string]string{ExternalTunnelCAAnnotation: "true"}
						}
						Expect(c.Create(ctx, managementCluster)).NotTo(HaveOccurred())
					...
					truthSecret := func() corev1.Secret {
					...
					installSecret := func() corev1.Secret {
						return corev1.Secret{
							TypeMeta: metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"},
							ObjectMeta: metav1.ObjectMeta{
								Name:      render.VoltronTunnelSecretName,
					...
					It("should wait rather than self-sign a CA when the tunnel secret is absent", func() {
						createManagementCluster(true)

						mockStatus.On("SetDegraded", operatorv1.ResourceNotReady,
							fmt.Sprintf("Waiting for the externally managed tunnel secret %s/%s to be created", common.OperatorNamespace(), render.VoltronTunnelSecretName),
							mock.Anything, mock.Anything).Return()

						result, err := r.Reconcile(ctx, reconcile.Request{})
						Expect(err).ShouldNot(HaveOccurred())
						Expect(result.RequeueAfter).To(BeNumerically(">", 0), "reconcile should requeue to wait for the secret")

						// The critical assertion: no CA was minted. Had the operator self-signed one, the cluster would
						// serve a certificate whose SAN is "voltron" and the delivery would be racing it.
						secret := truthSecret()
						Expect(kerror.IsNotFound(test.GetResource(c, &secret))).Should(BeTrue())
					})

					It("should adopt a pre-existing tunnel secret and still copy it for voltron to mount", func() {
						createManagementCluster(true)

						// Stand in for the sealed secret the cluster's argo application delivers.
						external, err := certificatemanagement.CreateSelfSignedSecret(render.VoltronTunnelSecretName, common.OperatorNamespace(), "tigera-voltron", []string{externalSAN})
						Expect(err).NotTo(HaveOccurred())
						Expect(c.Create(ctx, external)).NotTo(HaveOccurred())

						_, err = r.Reconcile(ctx, reconcile.Request{})
						Expect(err).ShouldNot(HaveOccurred())

						// The delivered key material and its SAN survive untouched, and the operator claims no
						// ownership -- taking it would fight sealed-secrets over this secret every reconcile.
						secret := truthSecret()
						Expect(test.GetResource(c, &secret)).NotTo(HaveOccurred())
						assertSANs(&secret, externalSAN)
						Expect(secret.Data).To(Equal(external.Data))
						Expect(secret.OwnerReferences).To(BeEmpty())

						// But the copy into the install namespace still happens: unlike multi-tenant, a single-tenant
						// install keeps its truth in tigera-operator while voltron mounts from the install namespace, so
						// skipping the copy would leave voltron with nothing.
						copied := installSecret()
						Expect(test.GetResource(c, &copied)).NotTo(HaveOccurred())
						assertSANs(&copied, externalSAN)
					})

					It("should still self-sign a CA for a cluster that is not annotated", func() {
						createManagementCluster(false)
		...
		Context("multi-tenant", func() {
			...
			Context("with an externally managed tunnel CA", func() {
				// A tenant whose tunnel CA is delivered from outside the operator -- in Calico Cloud, generated by tds
				// and applied as a sealed secret by the tenant's argo application. The operator must consume that CA
				// and never mint one of its own, because the externally generated CA carries the fully qualified DNS
				// SAN that the tenant's managed clusters are configured to dial.
				const externalSAN = "tenant-a.dev.calicocloud.io"

				annotateTenantA := func() {
					tenantA := &operatorv1.Tenant{}
					Expect(c.Get(ctx, types.NamespacedName{Name: "default", Namespace: tenantANamespace}, tenantA)).NotTo(HaveOccurred())
					tenantA.Annotations = map[string]string{ExternalTunnelCAAnnotation: "true"}
					Expect(c.Update(ctx, tenantA)).NotTo(HaveOccurred())
				}
				...
				It("should wait rather than self-sign a CA when the tunnel secret is absent", func() {
					...
					mockStatus.On("SetDegraded", operatorv1.ResourceNotReady,
						fmt.Sprintf("Waiting for the externally managed tunnel secret %s/%s to be created", tenantANamespace, render.VoltronTunnelSecretName),
						mock.Anything, mock.Anything).Return()
					...
					// The critical assertion: no CA was minted. Had the operator self-signed one, the tenant would be
					// serving a certificate whose SAN is the bare tenant ID and no managed cluster would connect.
					tunnelSecret := corev1.Secret{
						TypeMeta:   metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"},
						ObjectMeta: metav1.ObjectMeta{Name: render.VoltronTunnelSecretName, Namespace: tenantANamespace},
					}
					Expect(kerror.IsNotFound(test.GetResource(c, &tunnelSecret))).Should(BeTrue())
				})

				It("should adopt a pre-existing tunnel secret without rewriting it", func() {
					annotateTenantA()

					// Stand in for the sealed secret the tenant's argo application delivers.
					external, err := certificatemanagement.CreateSelfSignedSecret(render.VoltronTunnelSecretName, tenantANamespace, "tigera-voltron", []string{externalSAN})
					Expect(err).NotTo(HaveOccurred())
					Expect(c.Create(ctx, external)).NotTo(HaveOccurred())
					...
					Expect(test.GetResource(c, &tunnelSecret)).NotTo(HaveOccurred())

					// The delivered key material and its SAN survive untouched.
					assertSANs(&tunnelSecret, externalSAN)
					Expect(tunnelSecret.Data).To(Equal(external.Data))

					// And the operator claims no ownership of it, leaving the deliverer as the sole owner. Taking
					// ownership here is what would otherwise fight sealed-secrets over this secret each reconcile.
					Expect(tunnelSecret.OwnerReferences).To(BeEmpty())
				})

				It("should still self-sign a CA for a tenant that is not annotated", func() {
					_, err := r.Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Namespace: tenantANamespace}})
					Expect(err).ShouldNot(HaveOccurred())
					...
					Expect(test.GetResource(c, &tunnelSecret)).NotTo(HaveOccurred())
					assertSANs(&tunnelSecret, "tenant-a")
				})
			})
...
var _ = Describe("externallyManagedTunnelCA", func() {
	annotated := func(value string) map[string]string {
		return map[string]string{ExternalTunnelCAAnnotation: value}
	}

	DescribeTable("reading the annotation off the Tenant in multi-tenant mode",
		func(annotations map[string]string, expected bool) {
			tenant := &operatorv1.Tenant{ObjectMeta: metav1.ObjectMeta{Name: "default", Annotations: annotations}}
			Expect(externallyManagedTunnelCA(true, tenant, nil, logf.Log)).To(Equal(expected))
		},
		Entry("true", annotated("true"), true),
		// Truthy spellings strconv.ParseBool accepts. The annotation is written by tooling or by hand, so treating
		// only the exact literal "true" as enabled would silently fall back to self-signing the tunnel CA.
		Entry("True", annotated("True"), true),
		Entry("TRUE", annotated("TRUE"), true),
		Entry("1", annotated("1"), true),
		Entry("t", annotated("t"), true),
		Entry("false", annotated("false"), false),
		Entry("False", annotated("False"), false),
		Entry("0", annotated("0"), false),
		// Anything unparseable is treated as absent rather than as enabled: the operator keeps signing the CA
		// itself, which is what it did before the annotation existed.
		Entry("unparseable", annotated("yes-please"), false),
		Entry("empty", annotated(""), false),
		Entry("absent", map[string]string{}, false),
		Entry("no annotations at all", nil, false),
	)

	DescribeTable("reading the annotation off the ManagementCluster in single-tenant mode",
		func(annotations map[string]string, expected bool) {
			mc := &operatorv1.ManagementCluster{ObjectMeta: metav1.ObjectMeta{Name: "tigera-secure", Annotations: annotations}}
			Expect(externallyManagedTunnelCA(false, nil, mc, logf.Log)).To(Equal(expected))
		},
		Entry("true", annotated("true"), true),
		Entry("True", annotated("True"), true),
		Entry("1", annotated("1"), true),
		Entry("false", annotated("false"), false),
		Entry("unparseable", annotated("enabled"), false),
		Entry("absent", map[string]string{}, false),
	)

	It("should be false when the resource holding the annotation is missing", func() {
		// A multi-tenant install with no Tenant, and a single-tenant install with no ManagementCluster, both
		// have nowhere for the annotation to live.
		Expect(externallyManagedTunnelCA(true, nil, nil, logf.Log)).To(BeFalse())
		Expect(externallyManagedTunnelCA(false, nil, nil, logf.Log)).To(BeFalse())
	})

	It("should ignore the annotation on the resource the mode does not consult", func() {
		// Single-tenant mode reads the ManagementCluster only, and multi-tenant mode the Tenant only.
		tenant := &operatorv1.Tenant{ObjectMeta: metav1.ObjectMeta{Name: "default", Annotations: annotated("true")}}
		mc := &operatorv1.ManagementCluster{ObjectMeta: metav1.ObjectMeta{Name: "tigera-secure", Annotations: annotated("true")}}

		Expect(externallyManagedTunnelCA(false, tenant, nil, logf.Log)).To(BeFalse())
		Expect(externallyManagedTunnelCA(true, nil, mc, logf.Log)).To(BeFalse())
	})
})
