...
var _ = Describe("Parser", func() {
	...
	It("should detect an installation if one exists", func() {
		c := test.DefaultFakeClientBuilder(scheme).WithObjects(emptyNodeSpec(), emptyKubeControllerSpec(), pool, emptyFelixConfig()).Build()
		_, err := Convert(ctx, c)
	...
	It("should error if it detects a canal installation", func() {
		c := test.DefaultFakeClientBuilder(scheme).WithObjects(&appsv1.DaemonSet{
	...
	It("should error for unchecked env vars", func() {
		...
		c := test.DefaultFakeClientBuilder(scheme).WithObjects(node, emptyKubeControllerSpec(), pool, emptyFelixConfig()).Build()
		_, err := Convert(ctx, c)
		Expect(err).To(HaveOccurred())
	})
	...
	It("should detect an MTU via substitution", func() {
		...
		c := test.DefaultFakeClientBuilder(scheme).WithObjects(ds, emptyKubeControllerSpec(), pool, emptyFelixConfig()).Build()
	...
	Context("CNI", func() {
		var _ = Describe("CNI", func() {
			It("should load cni from correct fields on calico-node", func() {
				...
				cli := test.DefaultFakeClientBuilder(scheme).WithObjects(emptyKubeControllerSpec()).Build()
				c := components{
