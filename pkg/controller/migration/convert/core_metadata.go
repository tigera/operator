...
func handleAnnotations(c *components, install *operatorv1.Installation) error {
	...
	if len(annots) == 0 {
		if hasAnnotations(install.Spec.CalicoNodeDaemonSet) {
	...
	if len(annots) == 0 {
		if hasPodTemplateAnnotations(install.Spec.CalicoNodeDaemonSet) {
...
func handleLabels(c *components, install *operatorv1.Installation) error {
	...
	if len(labels) == 0 {
		if hasLabels(install.Spec.CalicoNodeDaemonSet) {
	...
	if len(labels) == 0 {
		if hasPodTemplateLabels(install.Spec.CalicoNodeDaemonSet) {
	...
	if c.kubeControllers == nil {
		...
		if len(labels) == 0 {
			if hasLabels(install.Spec.CalicoKubeControllersDeployment) {
		...
		if len(labels) == 0 {
			if hasPodTemplateLabels(install.Spec.CalicoKubeControllersDeployment) {
