...
var _ = Describe("Convert network tests", func() {
	...
	Describe("handle alternate CNI migration", func() {
		DescribeTable("non-calico plugins", func(envs []corev1.EnvVar, plugin operatorv1.CNIPluginType) {
			...
			c := test.DefaultFakeClientBuilder(scheme).WithObjects(ds, emptyKubeControllerSpec(), v4pool, emptyFelixConfig()).Build()
		...
		It("should convert AWS CNI install", func() {
			c := test.DefaultFakeClientBuilder(scheme).WithRuntimeObjects(append([]runtime.Object{v4pool, emptyFelixConfig(), getK8sNodes(6)}, awsCNIPolicyOnlyConfig()...)...).Build()
			_, err := Convert(ctx, c)
			Expect(err).NotTo(HaveOccurred())
		})
	})
	...
	Describe("handle Calico CNI migration", func() {
		It("migrate default", func() {
			c := test.DefaultFakeClientBuilder(scheme).WithObjects(emptyNodeSpec(), emptyKubeControllerSpec(), v4pool, emptyFelixConfig()).Build()
			cfg, err := Convert(ctx, c)
			Expect(err).ToNot(HaveOccurred())
			Expect(cfg).ToNot(BeNil())
		...
		It("should convert Calico v3.15 manifest", func() {
			c := test.DefaultFakeClientBuilder(scheme).WithRuntimeObjects(append([]runtime.Object{v4pool, emptyFelixConfig()}, calicoDefaultConfig()...)...).Build()
			cfg, err := Convert(ctx, c)
			Expect(err).NotTo(HaveOccurred())
		...
		It("migrate default dual stack config", func() {
			...
			c := test.DefaultFakeClientBuilder(scheme).WithObjects(v4pool, v6pool, ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			cfg, err := Convert(ctx, c)
			Expect(err).ToNot(HaveOccurred())
			Expect(cfg).ToNot(BeNil())
			Expect(cfg.Spec.CNI.Type).To(Equal(operatorv1.PluginCalico))
		...
		It("fails migrating default dual stack config if missing pools", func() {
			...
			c := test.DefaultFakeClientBuilder(scheme).WithObjects(v6pool, ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			cfg, err := Convert(ctx, c)
			...
			c = test.DefaultFakeClientBuilder(scheme).WithObjects(v4pool, ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			cfg, err = Convert(ctx, c)
			Expect(err).To(HaveOccurred())
		...
		It("migrate default IPv6 only config", func() {
			...
			bothPools := test.DefaultFakeClientBuilder(scheme).WithObjects(v4pool, v6pool, ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			runTest(bothPools)

			// Run test but with only v6 pool
			ipv6Only := test.DefaultFakeClientBuilder(scheme).WithObjects(v6pool, ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			runTest(ipv6Only)
		})
		It("fails migrating default IPv6 only config if missing pool", func() {
			...
			// no pools at all
			c := test.DefaultFakeClientBuilder(scheme).WithObjects(ds, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			cfg, err := Convert(ctx, c)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal("CNI config indicates assign_ipv6=true but there were no valid IPv6 pools found. To fix it, create an IPv6 pool or set assign_ipv6=false on cni-config"))
			Expect(cfg).To(BeNil())
			...
			// IPv4 pool only
			c = test.DefaultFakeClientBuilder(scheme).WithObjects(ds, v4pool, emptyKubeControllerSpec(), emptyFelixConfig()).Build()
			cfg, err = Convert(ctx, c)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal("CNI config indicates assign_ipv6=true but there were no valid IPv6 pools found. To fix it, create an IPv6 pool or set assign_ipv6=false on cni-config"))
			Expect(cfg).To(BeNil())
