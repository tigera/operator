...
func (r *ReconcileMonitor) readAlertmanagerConfigSecret(ctx context.Context) (*corev1.Secret, bool, error) {
	...
	//
	// TODO For future Alertmanager manager configuration upgrades, we need to keep previous versions of the configuration
	//   and compare it with the one in the cluster. If it matches one of the Tigera versions, we can safely upgrade it.
	//   Otherwise, we have to leave it as-is (unmanaged) because it is modified by the user.
