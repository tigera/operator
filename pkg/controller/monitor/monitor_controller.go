...
func (r *ReconcileMonitor) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// When the UI alerts integration is enabled, provision a service-account-token secret that the
	// Alertmanager webhook uses as its Linseed bearer token. Use a create-only passthrough so the
	// operator does not overwrite the token Kubernetes populates into it.
	if instance.Spec.UIAlertsEnabled() {
		components = append(components, render.NewCreationPassthrough(monitor.AlertmanagerLinseedTokenSecret()))
	}
