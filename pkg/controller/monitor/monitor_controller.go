...
func (r *ReconcileMonitor) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if _, err := r.getAlertmanagerService(ctx); err != nil {
		reqLogger.Error(err, err.Error())
		r.status.SetDegraded("Failed to retrieve Alertmanager service", err.Error())
		return reconcile.Result{}, err
	}

	if _, err := r.getPrometheusService(ctx); err != nil {
		reqLogger.Error(err, err.Error())
		r.status.SetDegraded("Failed to retrieve Prometheus service", err.Error())
		return reconcile.Result{}, err
	}

	if _, err := r.getAlertmanager(ctx); err != nil {
		reqLogger.Error(err, err.Error())
		r.status.SetDegraded("Failed to retrieve Alertmanager", err.Error())
		return reconcile.Result{}, err
	}

	if _, err := r.getPrometheus(ctx); err != nil {
		reqLogger.Error(err, err.Error())
		r.status.SetDegraded("Failed to retrieve Prometheus", err.Error())
		return reconcile.Result{}, err
	}
...
func (r *ReconcileMonitor) getAlertmanager(ctx context.Context) (*monitoringv1.Alertmanager, error) {
	am := monitoringv1.Alertmanager{}
	err := r.client.Get(ctx, client.ObjectKey{Name: render.CalicoNodeAlertmanager, Namespace: common.TigeraPrometheusNamespace}, &am)
	if err != nil {
		return nil, err
	}
...
func (r *ReconcileMonitor) getAlertmanagerService(ctx context.Context) (*corev1.Service, error) {
	svc := corev1.Service{}
	err := r.client.Get(ctx, client.ObjectKey{Name: render.CalicoNodeAlertmanager, Namespace: common.TigeraPrometheusNamespace}, &svc)
	if err != nil {
		return nil, err
	}
...
func (r *ReconcileMonitor) getPrometheus(ctx context.Context) (*monitoringv1.Prometheus, error) {
	p := monitoringv1.Prometheus{}
	err := r.client.Get(ctx, client.ObjectKey{Name: render.CalicoNodePrometheus, Namespace: common.TigeraPrometheusNamespace}, &p)
	if err != nil {
		return nil, err
	}
...
func (r *ReconcileMonitor) getPrometheusService(ctx context.Context) (*corev1.Service, error) {
	svc := corev1.Service{}
	err := r.client.Get(ctx, client.ObjectKey{Name: render.CalicoNodePrometheus, Namespace: common.TigeraPrometheusNamespace}, &svc)
	if err != nil {
		return nil, err
	}
