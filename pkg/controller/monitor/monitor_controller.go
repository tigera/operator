...
func (r *ReconcileMonitor) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	alertmanagerConfigSecret, setOwner, err := r.readOrCreateAlertmanagerConfigSecret(ctx)
	if err != nil {
...
// readOrCreateAlertmanagerConfigSecret attempts to retrieve Alertmanager configuration secret from either the Tigera Operator
...
func (r *ReconcileMonitor) readOrCreateAlertmanagerConfigSecret(ctx context.Context) (*corev1.Secret, bool, error) {
	...
	//   Tigera Operator holds the ownership of this secret.
	//
	// To handle upgrades:
	//   The tigera-prometheus secret will be copied back to the tigera-operator namespace.
	//   If this secret is modified by the user, the Operator won't set the ownership. Otherwise, it is owned by the Operator.
	//
	...
	// If it is modified by the user, Operator will not set the owner reference.
	...
	if secret != nil {
		...
		return rsecret.CopyToNamespace(common.OperatorNamespace(), secret)[0], false, nil
	}
	...
	if err := r.client.Create(ctx, defaultConfigSecret); err != nil {
		return nil, false, err
	}
