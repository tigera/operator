...
var _ = Describe("Monitor controller tests", func() {
	...
	Context("image reconciliation", func() {
		It("should monitor Premetheus Operator deployment", func() {
			_, err := r.Reconcile(ctx, reconcile.Request{})
			Expect(err).NotTo(HaveOccurred())

			args := []types.NamespacedName{{Name: "calico-prometheus-operator", Namespace: common.TigeraPrometheusNamespace}}
			Expect(mockStatus.AssertCalled(nil, "AddDeployments", args)).To(BeTrue())
			Expect(mockStatus.AssertCalled(nil, "ReadyToMonitor")).To(BeTrue())
		})
