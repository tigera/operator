...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	if err = utils.AddNetworkWatch(c); err != nil {
		return fmt.Errorf("monitoring-controller failed to watch Network resource: %w", err)
	}
...
func (r *ReconcileMonitoring) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	variant, installation, err := installation.GetInstallation(context.Background(), r.client)
	if err != nil {
		if errors.IsNotFound(err) {
	...
	component := render.Monitoring(installation)
