...
func newReconciler(mgr manager.Manager, opts options.ControllerOptions) (reconcile.Reconciler, error) {
	...
	// Both modes fan out to the non-cluster hosts, so both scale with the registered-host
	// (HostEndpoint) count: serval (typhaEndpoint unset), whose in-process Typha
	// serves the hosts, or the legacy calico-typha-noncluster-host deployment (typhaEndpoint
	// set). Exactly one exists at a time, so a single autoscaler drives both — it applies the
	// count to each and skips the one that is absent.
...
func add(mgr manager.Manager, c ctrlruntime.Controller) error {
	...
	// Serval's serving cert and its Typha client keypair drive a re-render.
	for _, secretName := range []string{serval.ServalKeyPairSecret, serval.ServalTyphaClientKeyPairSecret, certificatemanagement.CASecretName} {
...
type ReconcileNonClusterHost struct {
	...
	// autoscaler scales serval and the legacy calico-typha-noncluster-host
...
func (r *ReconcileNonClusterHost) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	// serval.
	tunnelMode := instance.Spec.TyphaEndpoint == ""
	if !tunnelMode {
		logc.Info("spec.typhaEndpoint is deprecated; clear it to reach Typha through the tunnel")
	...
	if tunnelMode {
		// Only tunnel mode needs an Installation (for images and certificates).
		...
		tunnelComponents, res, err := r.tunnelComponents(ctx, installationSpec, endpointHost, logc)
		if err != nil || res != nil {
			if res != nil {
				return *res, err
			}
		...
		components = append(components, tunnelComponents...)
...
// tunnelComponents builds the serval components (cert management + serval). It returns
// a non-nil *reconcile.Result when the caller should return early (e.g. waiting on a keypair).
func (r *ReconcileNonClusterHost) tunnelComponents(ctx context.Context, installationSpec *operatorv1.InstallationSpec, endpointHost string, logc logr.Logger) ([]render.Component, *reconcile.Result, error) {
	...
	// Serval relays felix tunnels to the in-cluster calico-typha as an ordinary
	// Typha client, so it needs a client keypair whose CN (typha-client)
	// calico-typha accepts, signed by the cluster CA.
	typhaClientKeyPair, err := certificateManager.GetOrCreateKeyPair(
		r.client, serval.ServalTyphaClientKeyPairSecret, common.OperatorNamespace(),
		[]string{render.FelixCommonName})
	if err != nil {
		r.status.SetDegraded(operatorv1.ResourceCreateError, "Error creating the Typha client certificate", err, logc)
		return nil, &reconcile.Result{}, err
	}
	...
	certComponent := rcertificatemanagement.CertificateManagement(&rcertificatemanagement.Config{
		...
		KeyPairOptions: []rcertificatemanagement.KeyPairOption{
			...
			rcertificatemanagement.NewKeyPairOption(typhaClientKeyPair, true, true),
		},
	...
	servalComponent := serval.Serval(&serval.Configuration{
		...
		TyphaClientKeyPair: typhaClientKeyPair,
