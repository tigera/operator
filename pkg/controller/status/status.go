...
func (m *statusManager) updateStatus() {
	...
	if m.enabled != nil && !*m.enabled {
		// This status manager is not enabled. For example, because the controller has not yet
		// called OnCRFound(). Remove any TigeraStatus object that had previously been created.
...
// removeTigeraStatus status displayed in TigeraStatus if corresponding controller's CR is not found
