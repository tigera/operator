...
func (r *ReconcileTiers) prepareTiersConfig(ctx context.Context, reqLogger logr.Logger) (*tiers.Config, *reconcile.Result) {
	...
	if r.provider != operatorv1.ProviderOpenShift {
		...
		if err != nil {
			...
			// kube-dns.
			kubeDNSServiceList := &corev1.ServiceList{}

			err = r.client.List(ctx, kubeDNSServiceList, &client.ListOptions{LabelSelector: labels.SelectorFromSet(map[string]string{"k8s-app": "kube-dns"})})
			if err != nil {
				if errors.IsNotFound(err) {
			...
			for _, service := range kubeDNSServiceList.Items {
