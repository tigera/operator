...
func (r *ReconcileTiers) prepareTiersConfig(ctx context.Context, reqLogger logr.Logger) (*tiers.Config, *reconcile.Result) {
	...
	if r.provider != operatorv1.ProviderOpenShift {
		...
		if err != nil {
			...
			if r.provider == operatorv1.ProviderRKE2 {
				dnsServiceName = "rke2-coredns-rke2-coredns"
			}

			kubeDNSService, err := utils.GetDNSServiceByName(dnsServiceName, ctx, r.client)
			if err != nil {
				if apierrors.IsNotFound(err) {
					r.status.SetDegraded(operatorv1.ResourceNotFound, fmt.Sprintf("%s service not found", dnsServiceName), err, reqLogger)
				} else {
					r.status.SetDegraded(operatorv1.ResourceReadError, fmt.Sprintf("Error querying %s service", dnsServiceName), err, reqLogger)
				}
				return nil, &reconcile.Result{RequeueAfter: 10 * time.Second}
			}

			// Discover the DNS Service's cluster IP address.
			var kubeDNSIPs = kubeDNSService.Spec.ClusterIPs
