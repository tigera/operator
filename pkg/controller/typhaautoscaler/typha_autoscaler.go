...
// and Serval. Controllers own the deployment they scale; this
...
const (
	...
	// nonClusterHostLabelKey/Value tag a HostEndpoint that a non-cluster host
	// registered (the node init's label updater sets them). Only these count
	// toward scaling: auto host endpoints and in-cluster ones are not served by
	// serval.
	nonClusterHostLabelKey   = "hostendpoint.projectcalico.org/type"
	nonClusterHostLabelValue = "nonclusterhost"
)
...
// deployment from the same count — the non-cluster-host controller scales serval and
...
// the node count. Use it for the non-cluster-host Typha and Serval.
...
// Start to begin scaling. Used by the non-cluster-host controller, which drives both
// serval and the legacy Typha deployment from one count.
...
// getHostEndpointCounts returns the number of HostEndpoints that non-cluster hosts registered,
// identified by the label the node init sets. Auto host endpoints and in-cluster ones are excluded.
func (t *Autoscaler) getHostEndpointCounts() int {
	heps := 0
	for _, obj := range t.indexInformer.GetIndexer().List() {
		hep := obj.(*v3.HostEndpoint)
		if hep.Labels[nonClusterHostLabelKey] == nonClusterHostLabelValue {
			heps++
		}
	}
	return heps
