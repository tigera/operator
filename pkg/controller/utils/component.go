...
type ComponentHandler interface {
	CreateOrUpdate(context.Context, render.Component, status.StatusManager, AllowedMetadaKeys) error
}
...
func (c componentHandler) CreateOrUpdate(ctx context.Context, component render.Component, status status.StatusManager, allowedMetadata AllowedMetadaKeys) error {
...
func mergeState(desired, current runtime.Object, allowedMetadata AllowedMetadaKeys) runtime.Object {
...
// AllowedMetadaKeys contains slices of keys of certain annotations or labels
// that are allowed to be set on objects.
type AllowedMetadaKeys struct {
	Annotations []string
	Labels      []string
}
...
var (
	...
	AllowOpenshiftSCCAnnotations = AllowedMetadaKeys{
		Annotations: []string{
	...
	NoUserAddedMetadata = AllowedMetadaKeys{}
)

// copyAllowedMetadata copies AllowedMetadaKeys from current into desired ObjectMeta. The desired ObjectMeta will be updated
// in-place.
func copyAllowedMetadata(metadataKeys AllowedMetadaKeys, current, desired metav1.Object) {
	mergedAnnotations := mergeAnnotationsOrLabels(metadataKeys.Annotations, current.GetAnnotations(), desired.GetAnnotations())
	desired.SetAnnotations(mergedAnnotations)
	mergedLabels := mergeAnnotationsOrLabels(metadataKeys.Labels, current.GetLabels(), desired.GetLabels())
