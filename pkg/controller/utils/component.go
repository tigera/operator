...
//
// For liveness probe: timeout to 5s and period to 60s so that one component will be restarted after
// around 3 minutes (3 default failure threshold).
// For readiness probe: timeout to 5s and period to 30s so that one component will be removed from
// service after 1.5 minute (3 default failure threshold).
