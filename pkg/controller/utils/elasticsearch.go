...
// resetClient reset the Elasticsearch client.
...
func (es *esClient) createOrUpdatePolicies(ctx context.Context, client client.Client, listPolicy map[string]policyDetail) error {
	for indexName, pd := range listPolicy {
		...
		if err != nil {
			...
			// reset the Elasticsearch client to handle cases where Elasticsearch gets recreated
			// or Elasticsearch credential gets recreated.
			es.resetClient()
			return err
		}
