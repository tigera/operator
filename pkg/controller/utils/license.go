...
// FetchLicenseKeyWithGracePeriod fetches the LicenseKey resource using a single unstructured Get
// and returns both the typed license and the grace period duration. The grace period is read from
// status.gracePeriod (a string representing the number of days) which is not present in the Go API
// type. The grace period defaults to 0 if the field is not set or cannot be parsed.
func FetchLicenseKeyWithGracePeriod(ctx context.Context, cli client.Client) (v3.LicenseKey, time.Duration, error) {
	...
	if err := cli.Get(ctx, DefaultInstanceKey, u); err != nil {
		return v3.LicenseKey{}, 0, err
	}

	// Extract grace period from the unstructured status before converting.
	gracePeriod := parseGracePeriod(u)

	// Convert unstructured to typed LicenseKey.
	var license v3.LicenseKey
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.Object, &license); err != nil {
		return v3.LicenseKey{}, 0, fmt.Errorf("failed to convert unstructured LicenseKey: %w", err)
	}

	return license, gracePeriod, nil
}

// parseGracePeriod extracts the grace period from an unstructured LicenseKey object.
// Returns 0 if the field is not set or cannot be parsed.
func parseGracePeriod(u *unstructured.Unstructured) time.Duration {
	status, ok := u.Object["status"].(map[string]interface{})
	if !ok {
		return 0
	}
	gpStr, ok := status["gracePeriod"].(string)
	if !ok || gpStr == "" {
		return 0
	}
	days, err := strconv.Atoi(gpStr)
	if err != nil {
		return 0
	}
