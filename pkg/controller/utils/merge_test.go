...
var _ = Describe("Installation merge tests", func() {
	...
	Context("test APIServerDeployment merge", func() {
		...
		DescribeTable("merge metadata", func(main, second, expect *opv1.Metadata) {
			...
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment).To(BeNil())
			} else {
				Expect(*inst.APIServerDeployment.Metadata).To(Equal(*expect))
			}
		}, metadataTests...)
		...
		DescribeTable("merge pod template metadata", func(main, second, expect *opv1.Metadata) {
			...
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment.Spec.Template.Metadata).To(BeNil())
			} else {
				Expect(*inst.APIServerDeployment.Spec.Template.Metadata).To(Equal(*expect))
			}
		}, metadataTests...)
		...
		_apiServerInit1a := opv1.APIServerDeploymentInitContainer{Name: "init1", Resources: _resources1}
		_apiServerInit1b := opv1.APIServerDeploymentInitContainer{Name: "init1", Resources: _resources2}
		_apiServerInit2 := opv1.APIServerDeploymentInitContainer{Name: "init2", Resources: _resources2}

		DescribeTable("merge initContainers", func(main, second, expect []opv1.APIServerDeploymentInitContainer) {
			m.APIServerDeployment.Spec.Template.Spec.InitContainers = main
			s.APIServerDeployment.Spec.Template.Spec.InitContainers = second
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.CalicoNodeDaemonSet.Spec.Template.Spec.InitContainers).To(BeNil())
			} else {
			...
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}, nil, []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}),
			Entry("Second only set", nil, []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}, []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}),
			Entry("Both set equal", []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}, []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2},
				[]opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}),
			Entry("Both set not equal", []opv1.APIServerDeploymentInitContainer{_apiServerInit1a, _apiServerInit2}, []opv1.APIServerDeploymentInitContainer{_apiServerInit1b, _apiServerInit2},
				[]opv1.APIServerDeploymentInitContainer{_apiServerInit1b, _apiServerInit2}),
		)

		_apiServer1a := opv1.APIServerDeploymentContainer{Name: "apis1"}
		_apiServer1b := opv1.APIServerDeploymentContainer{Name: "apis1"}
		_apiServer2 := opv1.APIServerDeploymentContainer{Name: "apis2"}

		DescribeTable("merge containers", func(main, second, expect []opv1.APIServerDeploymentContainer) {
			m.APIServerDeployment.Spec.Template.Spec.Containers = main
			s.APIServerDeployment.Spec.Template.Spec.Containers = second
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.CSINodeDriverDaemonSet.Spec.Template.Spec.Containers).To(BeNil())
			} else {
			...
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}, nil, []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}),
			Entry("Second only set", nil, []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}, []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}),
			Entry("Both set equal", []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}, []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b},
				[]opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer1b}),
			Entry("Both set not equal", []opv1.APIServerDeploymentContainer{_apiServer1a, _apiServer2}, []opv1.APIServerDeploymentContainer{_apiServer1b, _apiServer2},
				[]opv1.APIServerDeploymentContainer{_apiServer1b, _apiServer2}),
		)
		...
		DescribeTable("merge affinity", func(main, second, expect *v1.Affinity) {
			...
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.Affinity).To(BeNil())
			} else {
				Expect(*inst.APIServerDeployment.Spec.Template.Spec.Affinity).To(Equal(*expect))
			}
		},
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", _aff1, nil, _aff1),
		...
		DescribeTable("merge nodeSelector", func(main, second, expect map[string]string) {
			...
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.NodeSelector).To(BeNil())
			} else {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.NodeSelector).To(Equal(expect))
			}
		},
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", map[string]string{"a1": "1"}, nil, map[string]string{"a1": "1"}),
		...
		DescribeTable("merge topology spread constraints", func(main, second, expect []v1.TopologySpreadConstraint) {
			m.APIServerDeployment.Spec.Template.Spec.TopologySpreadConstraints = main
			s.APIServerDeployment.Spec.Template.Spec.TopologySpreadConstraints = second
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.TopologySpreadConstraints).To(BeNil())
			} else {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.TopologySpreadConstraints).To(Equal(expect))
			}
		},
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", []v1.TopologySpreadConstraint{_topology1}, nil, []v1.TopologySpreadConstraint{_topology1}),
			Entry("Second only set", nil, []v1.TopologySpreadConstraint{_topology1}, []v1.TopologySpreadConstraint{_topology1}),
			Entry("Both set equal", []v1.TopologySpreadConstraint{_topology1}, []v1.TopologySpreadConstraint{_topology1}, []v1.TopologySpreadConstraint{_topology1}),
			Entry("Both set not equal", []v1.TopologySpreadConstraint{_topology1}, []v1.TopologySpreadConstraint{_topology2}, []v1.TopologySpreadConstraint{_topology2}),
			Entry("Both set not equal, override empty", []v1.TopologySpreadConstraint{_topology1}, []v1.TopologySpreadConstraint{}, []v1.TopologySpreadConstraint{}),
		)
		...
		DescribeTable("merge tolerations", func(main, second, expect []v1.Toleration) {
			...
			inst := OverrideInstallationSpec(m, s)
			if expect == nil {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.Tolerations).To(BeNil())
			} else {
				Expect(inst.APIServerDeployment.Spec.Template.Spec.Tolerations).To(Equal(expect))
			}
		},
			Entry("Both unset", nil, nil, nil),
			Entry("Main only set", []v1.Toleration{_toleration1}, nil, []v1.Toleration{_toleration1}),
		...
		DescribeTable("merge multiple APIServerDeployment fields", func(main, second, expect *opv1.APIServerDeployment) {
			// start with empty spec
			m = opv1.InstallationSpec{}
			s = opv1.InstallationSpec{}
			if main != nil {
			...
			if expect == nil {
				...
				Expect(*inst.APIServerDeployment).To(Equal(*expect))
			}
		},
			Entry("Both unset", nil, nil, nil),
			Entry("Different fields in the two are merged, some overridden",
				&opv1.APIServerDeployment{
					...
					Spec: &opv1.APIServerDeploymentSpec{
						...
						Template: &opv1.APIServerDeploymentPodTemplateSpec{
							Spec: &opv1.APIServerDeploymentPodSpec{
								Containers:   []opv1.APIServerDeploymentContainer{_apiServer1a},
								NodeSelector: map[string]string{"selector": "test"},
								Tolerations:  []v1.Toleration{_toleration1},
							},
						},
