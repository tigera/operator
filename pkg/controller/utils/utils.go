...
func WaitToAddResourceWatch(controller controller.Controller, client kubernetes.Interface, log logr.Logger, flag *ReadyFlag, obj client.Object) {
	...
	defer ticker.Stop()
	log.Info("handlng watch of resource %s", obj.GetObjectKind().GroupVersionKind().Kind)
	for {
		select {
		case <-ticker.C:
			duration = duration * 2
			...
			if ok, err := isResourceReady(client, obj.GetObjectKind().GroupVersionKind().Kind); err != nil {
				log.Info("failed to list registered groups and resources - will retry: %v", err)
			} else if !ok {
				log.Info("waiting for %s resource to be registered - will retry", obj.GetObjectKind().GroupVersionKind().Kind)
			} else if err := controller.Watch(&source.Kind{Type: obj}, &handler.EnqueueRequestForObject{}); err != nil {
				log.Info("failed to watch %s resource - will retry: %v", obj.GetObjectKind().GroupVersionKind().Kind, err)
			} else {
				flag.MarkAsReady()
				return
			}
