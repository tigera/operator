...
func WaitToAddResourceWatch(controller controller.Controller, client kubernetes.Interface, log logr.Logger, flag *ReadyFlag, obj client.Object) {
	...
	defer ticker.Stop()
	logCxt := log.WithField("Kind", obj.GetObjectKind().GroupVersionKind().Kind)
	logCxt.Info("Waiting for API to become available for watching")
	for {
		select {
		case <-ticker.C:
			duration = duration * 2
			...
			if ok, err := isResourceReady(client, obj.GetObjectKind().GroupVersionKind().Kind); err != nil {
				logCxt.WithError(err).Info("Failed to check if resource is ready - will retry")
			} else if !ok {
				logCxt.Info("Waiting for resource to be ready - will retry")
			} else if err := controller.Watch(&source.Kind{Type: obj}, &handler.EnqueueRequestForObject{}); err != nil {
				logCxt.WithError(err).Info("Failed to watch resource - will retry")
			} else {
				logCxt.Info("Successfully watching resource")
				flag.MarkAsReady()
				return
			}
		}
