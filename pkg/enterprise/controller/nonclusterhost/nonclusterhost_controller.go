...
func (r *ReconcileNonClusterHost) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.Spec.IngestionCompression == nil {
		// Hosts read this CR directly, and the CRD default only applies on
		// write, so materialize the default for CRs created before the field
		// existed to make the effective value visible.
