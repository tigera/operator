...
func TestWAFBFFComponents_HasExpectedKinds(t *testing.T) {
	objs := applicationlayer.WAFBFFComponents(&minimalInstallation, "tigera/waf-bff:v0.1.0", &fakeCertPair{})
	got := map[string]int{}
	for _, o := range objs {
		got[o.GetObjectKind().GroupVersionKind().Kind]++
	}
	...
	require.Equal(t, 1, got["Service"], "expected 1 Service")
	require.Equal(t, 1, got["ServiceAccount"], "expected 1 ServiceAccount")
	require.Len(t, objs, 3, "expected exactly 3 objects")
}

func TestWAFFrontendModuleRegistration_HasConfigMap(t *testing.T) {
	objs := applicationlayer.WAFFrontendModuleRegistration(&minimalInstallation)
	require.Len(t, objs, 1, "expected exactly 1 object")
	require.Equal(t, "ConfigMap", objs[0].GetObjectKind().GroupVersionKind().Kind)
}
