...
// accepted with the scheme's default port, so a field documented as host:port
// fails loudly when given one.
...
// already parsed: an exact net for a literal IP, otherwise the domain.
//
// Service matching is opt-in via clusterDomain, because a Services match cannot
// carry ports. Callers whose destination is never in-cluster, or that need their
// ports preserved, pass an empty clusterDomain.
//
// The returned rule always constrains the destination. A rule carrying only
// ports would allow every host on those ports; callers that cannot use the rule
// they get are expected to drop it rather than widen it.
...
// Service DNS name. The host must be <service>.<namespace>.svc, optionally
// followed by the cluster domain and a trailing dot. Anything longer is an
// external host that happens to carry an "svc" label -- proxy.corp.svc.example.com
// is not a Service, and matching it as one would retarget the rule.
