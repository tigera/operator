...
var _ = Describe("compliance rendering tests", func() {
	Context("Standalone cluster", func() {
		It("should render all resources for a default configuration", func() {
			...
			}, nil, render.NewElasticsearchClusterConfig("cluster", 1, 1, 1), nil, notOpenshift, nil, nil, nil, nil)
	...
	Context("Management cluster", func() {
		It("should render all resources for a default configuration", func() {
			component, err := render.Compliance(nil, &internalManagerTLSSecret,
				...
				}, nil, render.NewElasticsearchClusterConfig("cluster", 1, 1, 1), nil, notOpenshift, &operatorv1.ManagementCluster{}, nil, nil, nil)
			Expect(err).ShouldNot(HaveOccurred())
			resources, _ := component.Objects()
	...
	Context("ManagedCluster", func() {
		It("should render all resources for a default configuration", func() {
			...
			}, nil, render.NewElasticsearchClusterConfig("cluster", 1, 1, 1), nil, notOpenshift, nil, &operatorv1.ManagementClusterConnection{}, nil, nil)
			Expect(err).ShouldNot(HaveOccurred())
			resources, _ := component.Objects()
