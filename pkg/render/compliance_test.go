...
var _ = Describe("compliance rendering tests", func() {
	...
	Describe("node selection & affinity", func() {
		var renderCompliance = func(i *operatorv1.InstallationSpec) (server, controller, snapshotter *v1.Deployment, reporter *corev1.PodTemplate, benchmarker *v1.DaemonSet) {
			component, err := render.Compliance(nil, nil, i, nil, render.NewElasticsearchClusterConfig("cluster", 1, 1, 1), nil, notOpenshift, nil, nil, nil)
			Expect(err).ShouldNot(HaveOccurred())
			resources, _ := component.Objects()
			server = GetResource(resources, "compliance-server", ns, "apps", "v1", "Deployment").(*v1.Deployment)
			controller = GetResource(resources, "compliance-controller", ns, "apps", "v1", "Deployment").(*v1.Deployment)
			snapshotter = GetResource(resources, "compliance-snapshotter", ns, "apps", "v1", "Deployment").(*v1.Deployment)
			reporter = GetResource(resources, "tigera.io.report", ns, "", "v1", "PodTemplate").(*corev1.PodTemplate)
			benchmarker = GetResource(resources, "compliance-benchmarker", ns, "apps", "v1", "DaemonSet").(*v1.DaemonSet)
			return
		}
