...
var _ = Describe("compliance rendering tests", func() {
	...
	Context("Management cluster", func() {
		It("should render all resources for a default configuration", func() {
			...
			Expect(dpComplianceServer.Spec.Template.Spec.Containers[0].VolumeMounts[2].Name).To(Equal(certificatemanagement.TrustedCertConfigMapName))
			Expect(dpComplianceServer.Spec.Template.Spec.Containers[0].VolumeMounts[2].MountPath).To(Equal(certificatemanagement.TrustedCertVolumeMountPath))
			...
			Expect(dpComplianceServer.Spec.Template.Spec.Volumes[2].Name).To(Equal(certificatemanagement.TrustedCertConfigMapName))
	...
	Context("Render Benchmarker", func() {
		...
		It("should render benchmarker properly for GKE environments", func() {
			...
			Expect(volumeMounts[6].Name).To(Equal(certificatemanagement.TrustedCertConfigMapName))
			Expect(volumeMounts[6].MountPath).To(Equal(certificatemanagement.TrustedCertVolumeMountPath))
		})
	})
