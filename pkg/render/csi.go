...
func (c *csiComponent) Objects() (objsToCreate, objsToDelete []client.Object) {
	...
	// (dataplane disabled) it has no purpose, so it is removed along with the other cases.
	if c.cfg.Terminating || c.cfg.Installation.KubeletVolumePluginPath == "None" || c.cfg.Installation.DataplaneDisabled() {
		objsToDelete = objs
	} else {
		objsToCreate = objs
	}
