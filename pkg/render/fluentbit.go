...
func (c *fluentBitComponent) eksLogForwarderDeployment() *appsv1.Deployment {
	...
	// StreamPrefix and FetchInterval are optional with no CRD default: omit
	// the env vars when unset so the plugin's envconfig defaults apply
	// (kube-apiserver-audit- / 10s). Rendering an empty prefix or a zero
	// interval would override those defaults with a broken setting.
