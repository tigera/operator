...
func (c *fluentdComponent) ResolveImages(is *operatorv1.ImageSet) error {
	...
	if c.osType == rcommon.OSTypeWindows {
		var err error
...
func (c *fluentdComponent) SupportedOSType() rcommon.OSType {
	return c.osType
}
...
func (c *fluentdComponent) Objects() ([]client.Object, []client.Object) {
	...
	objs = append(objs, rcommon.SecretsToRuntimeObjects(rcommon.CopySecrets(LogCollectorNamespace, c.pullSecrets...)...)...)
	if c.s3Credential != nil {
		objs = append(objs, c.s3CredentialSecret())
	}
	if c.splkCredential != nil {
		objs = append(objs, rcommon.SecretsToRuntimeObjects(rcommon.CopySecrets(LogCollectorNamespace, c.splunkCredentialSecret()...)...)...)
	}
	if c.filters != nil {
	...
	if c.eksConfig != nil {
		...
		if c.installation.KubernetesProvider != operatorv1.ProviderOpenShift && c.osType == rcommon.OSTypeLinux {
			objs = append(objs,
	...
	objs = append(objs, rcommon.SecretsToRuntimeObjects(rcommon.CopySecrets(LogCollectorNamespace, c.esSecrets...)...)...)
...
func (c *fluentdComponent) daemonset() *appsv1.DaemonSet {
	...
	if c.s3Credential != nil {
		annots[s3CredentialHashAnnotation] = rcommon.AnnotationHash(c.s3Credential)
	}
	if c.splkCredential != nil {
		annots[splunkCredentialHashAnnotation] = rcommon.AnnotationHash(c.splkCredential)
	}
	if c.filters != nil {
		annots[filterHashAnnotation] = rcommon.AnnotationHash(c.filters)
	}
...
func (c *fluentdComponent) envvars() []corev1.EnvVar {
	...
	if c.lc.Spec.AdditionalStores != nil {
		...
		if syslog != nil {
			proto, host, port, _ := rcommon.ParseEndpoint(syslog.Endpoint)
			envs = append(envs,
		...
		if splunk != nil {
			proto, host, port, _ := rcommon.ParseEndpoint(splunk.Endpoint)
			envs = append(envs,
...
func (c *fluentdComponent) eksLogForwarderDeployment() *appsv1.Deployment {
	annots := map[string]string{
		eksCloudwatchLogCredentialHashAnnotation: rcommon.AnnotationHash(c.eksConfig),
	}
	...
	envVars := []corev1.EnvVar{
		...
		{Name: "AWS_ACCESS_KEY_ID", ValueFrom: rcommon.EnvVarSourceFromSecret(EksLogForwarderSecret, EksLogForwarderAwsId, false)},
		{Name: "AWS_SECRET_ACCESS_KEY", ValueFrom: rcommon.EnvVarSourceFromSecret(EksLogForwarderSecret, EksLogForwarderAwsKey, false)},
	}
