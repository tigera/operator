...
var _ = Describe("Tigera Secure Fluentd rendering tests", func() {
	...
	DescribeTable("should render with a default configuration", func(clusterDomain, expectedElasticHost string) {
		...
		component := render.Fluentd(instance, nil, esConfigMap, s3Creds, splkCreds, filters, eksConfig, nil, installation, clusterDomain, rutil.OSTypeLinux)
		...
		ds := rtestutil.GetResource(resources, "fluentd-node", "tigera-fluentd", "apps", "v1", "DaemonSet").(*apps.DaemonSet)
	...
	It("should render for Windows nodes", func() {
		...
		// Should render the correct resources.
		component := render.Fluentd(instance, nil, esConfigMap, s3Creds, splkCreds, filters, eksConfig, nil, installation, dns.DefaultClusterDomain, rutil.OSTypeWindows)
		resources, _ := component.Objects()
		Expect(len(resources)).To(Equal(len(expectedResources)))
		...
		ds := rtestutil.GetResource(resources, "fluentd-node-windows", "tigera-fluentd", "apps", "v1", "DaemonSet").(*apps.DaemonSet)
	...
	It("should render with S3 configuration", func() {
		...
		// Should render the correct resources.
		component := render.Fluentd(instance, nil, esConfigMap, s3Creds, splkCreds, filters, eksConfig, nil, installation, dns.DefaultClusterDomain, rutil.OSTypeLinux)
		resources, _ := component.Objects()
		Expect(len(resources)).To(Equal(len(expectedResources)))
	...
	It("should render with EKS Cloudwatch Log", func() {
		...
		deploy := rtestutil.GetResource(resources, "eks-log-forwarder", "tigera-fluentd", "apps", "v1", "Deployment").(*apps.Deployment)
