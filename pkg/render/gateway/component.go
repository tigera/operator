...
// cluster-wide ClusterRole keeps the read verbs, which the controller-runtime
// cache needs in every namespace, and the write verbs are confined to the
// namespaces the user configured.
//
// The companion chart changes that drop create/update/delete from that
// ClusterRole live in projectcalico/calico and tigera/calico-private. Until both
// land the operator still holds the cluster-wide verbs and these objects grant
...
func (c *gatewayDeletionComponent) Objects() (objsToCreate, objsToDelete []client.Object) {
	...
	// Deleting these resources needs the same write permissions creating them did,
	// and the Role may be gone — a user pruning RBAC, or a namespace recreated.
	// The handler applies objsToCreate before objsToDelete, so re-rendering the
	// Roles here re-grants the operator before the deletes are issued. Both
	// namespaces are re-granted even on a move, because a move whose target is
	// the backend namespace still deletes the ReferenceGrant there.
	regrant := writerObjects(prefix, gwNS)
	if gwNS != bkNS {
		regrant = append(regrant, writerObjects(prefix, bkNS)...)
	}

	// The grant is then dropped, after the deletes above, so a failure part-way
	// leaves it in place for the next reconcile. On a move the backend namespace
