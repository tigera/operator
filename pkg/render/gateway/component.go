...
func (c *gatewayComponent) Objects() (objsToCreate, objsToDelete []client.Object) {
	...
	objs = append(objs, role(prefix, c.cfg.GatewayNamespace), roleBinding(prefix, c.cfg.GatewayNamespace))
	if c.cfg.GatewayNamespace != c.cfg.BackendNamespace {
		// The Backend and ReferenceGrant are written in the backend namespace.
		objs = append(objs, role(prefix, c.cfg.BackendNamespace), roleBinding(prefix, c.cfg.BackendNamespace))
	}
...
func role(resourcePrefix, namespace string) *rbacv1.Role {
	return &rbacv1.Role{
...
func roleBinding(resourcePrefix, namespace string) *rbacv1.RoleBinding {
...
func (c *gatewayDeletionComponent) Objects() (objsToCreate, objsToDelete []client.Object) {
	...
	objs = append(objs, roleBinding(prefix, gwNS), role(prefix, gwNS))
	if gwNS != bkNS && !move {
		objs = append(objs, roleBinding(prefix, bkNS), role(prefix, bkNS))
	}
