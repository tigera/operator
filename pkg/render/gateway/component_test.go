...
var _ = Describe("Gateway component render", func() {
		...
		Context("when Enterprise is false", func() {
			...
			It("skips SA and RoleBinding but keeps the proxy NetworkPolicy", func() {
				for _, obj := range toCreate {
					...
					if rb, ok := obj.(*rbacv1.RoleBinding); ok && rb.Name != gateway.WriterRoleName(prefix) {
	...
	Context("cross-namespace (gateway != backend)", func() {
		...
		It("grants itself write access in both namespaces it writes to", func() {
			writerName := gateway.WriterRoleName(prefix)
		...
		It("leaves SA, RoleBinding, and NetworkPolicy to the GatewayAPI controller even when Enterprise is true", func() {
			for _, obj := range toCreate {
				...
				case *rbacv1.RoleBinding:
					Expect(o.Name).To(Equal(gateway.WriterRoleName(prefix)),
...
var _ = Describe("Gateway deletion component", func() {
	...
	Context("same namespace", func() {
		It("re-grants write access before deleting, and drops it afterwards", func() {
			writerName := gateway.WriterRoleName(prefix)
			Expect(findObject[*rbacv1.Role](toCreate, writerName, gwNS)).NotTo(BeNil(),
				"the Role may have been pruned; cleanup re-grants itself before issuing deletes")
			Expect(findObject[*rbacv1.RoleBinding](toCreate, writerName, gwNS)).NotTo(BeNil())

			// Last two deletes: the RoleBinding then the Role, so losing the
			// grant cannot strand an earlier delete.
	...
	Context("namespace move cleanup", func() {
		...
		Context("moving into the backend namespace", func() {
			...
			It("re-grants the backend namespace, whose ReferenceGrant it deletes", func() {
				// Without this the delete above has no grant behind it on a
				// cluster that never had one, and the reconcile wedges: the
				// stray cleanup errors before the render can create it.
				writerName := gateway.WriterRoleName(prefix)
				Expect(findObject[*rbacv1.Role](toCreate, writerName, bkNS)).NotTo(BeNil())
				Expect(findObject[*rbacv1.RoleBinding](toCreate, writerName, bkNS)).NotTo(BeNil())
			})
		})
