...
func (d *dpiComponent) Objects() (objsToCreate, objsToDelete []client.Object) {
	...
	// Either all of these values are set or none is set when execution reaches here.
	// Add DeepPacketInspection related objects if all necessary bit of TLS config are set.
	addDPIDaemonSet := d.cfg.NodeTLSSecret != nil && d.cfg.TyphaTLSSecret != nil && d.cfg.TyphaCAConfigMap != nil
	...
	if addDPIDaemonSet && !d.cfg.HasNoLicense {
		...
		toCreate = append(toCreate, d.cfg.IntrusionDetection)
...
func (d *dpiComponent) defaultIntrusionDetectionComponentResources() {
	if d.cfg.IntrusionDetection.Spec.ComponentResources == nil {
		d.cfg.IntrusionDetection.Spec.ComponentResources = []operatorv1.IntrusionDetectionComponentResource{
			{
