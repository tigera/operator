...
func (c *kubeControllersComponent) controllersDeployment() *apps.Deployment {
	...
	image := components.GetReference(components.ComponentCalicoKubeControllers, c.cr.Spec.Registry, RefByDigest)
	if c.cr.Spec.Variant == operator.TigeraSecureEnterprise {
		image = components.GetReference(components.ComponentTigeraKubeControllers, c.cr.Spec.Registry, RefByDigest)
	}
