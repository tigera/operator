...
func (es *elasticsearchComponent) SupportedOSType() rutil.OSType {
...
func (es *elasticsearchComponent) Objects() ([]client.Object, []client.Object) {
	...
	if es.managementClusterConnection == nil {
		...
		toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(rutil.CopySecrets(ECKOperatorNamespace, es.pullSecrets...)...)...)
		...
		if len(es.pullSecrets) > 0 {
			toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(rutil.CopySecrets(ElasticsearchNamespace, es.pullSecrets...)...)...)
		}

		if len(es.elasticsearchSecrets) > 0 {
			toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(es.elasticsearchSecrets...)...)
		}
		...
		if len(es.pullSecrets) > 0 {
			toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(rutil.CopySecrets(KibanaNamespace, es.pullSecrets...)...)...)
		}

		if len(es.kibanaSecrets) > 0 {
			toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(es.kibanaSecrets...)...)
		}
		...
		if len(es.curatorSecrets) > 0 {
			toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(rutil.CopySecrets(ElasticsearchNamespace, es.curatorSecrets...)...)...)
	...
	if es.supportsOIDC() {
		toCreate = append(toCreate, rutil.SecretsToRuntimeObjects(es.dexCfg.RequiredSecrets(ElasticsearchNamespace)...)...)
	}
...
func (es elasticsearchComponent) podTemplate() corev1.PodTemplateSpec {
	...
	if es.provider == operatorv1.ProviderOpenShift {
		esContainer.SecurityContext = &corev1.SecurityContext{
			RunAsUser: types.Int64ToPtr(1000),
		}
	}
	...
	initOSSettingsContainer := corev1.Container{
		...
		SecurityContext: &corev1.SecurityContext{
			Privileged: types.BoolToPtr(true),
	...
	annotations := map[string]string{
		ElasticsearchTLSHashAnnotation: rutil.SecretsAnnotationHash(es.elasticsearchSecrets...),
	}
	if es.supportsOIDC() {
		initKeystore := corev1.Container{
			...
			Image: es.esImage,
			SecurityContext: &corev1.SecurityContext{
				Privileged: types.BoolToPtr(false),
			},
	...
	podTemplate := corev1.PodTemplateSpec{
		...
		Spec: corev1.PodSpec{
			...
			ImagePullSecrets:   rutil.GetImagePullSecretReferenceList(es.pullSecrets),
		...
		Spec: kbv1.KibanaSpec{
			...
			PodTemplate: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: KibanaNamespace,
					Annotations: map[string]string{
						KibanaTLSAnnotationHash: rutil.SecretsAnnotationHash(es.kibanaSecrets...),
					},
					Labels: map[string]string{
						"name":    KibanaName,
						"k8s-app": KibanaName,
