...
func (es *elasticsearchComponent) Objects() ([]client.Object, []client.Object) {
	...
	if es.managementClusterConnection == nil {
		...
		toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(rdata.CopySecrets(ECKOperatorNamespace, es.pullSecrets...)...)...)
		...
		if len(es.pullSecrets) > 0 {
			toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(rdata.CopySecrets(ElasticsearchNamespace, es.pullSecrets...)...)...)
		}

		if len(es.elasticsearchSecrets) > 0 {
			toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(es.elasticsearchSecrets...)...)
		}
		...
		if len(es.pullSecrets) > 0 {
			toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(rdata.CopySecrets(KibanaNamespace, es.pullSecrets...)...)...)
		}

		if len(es.kibanaSecrets) > 0 {
			toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(es.kibanaSecrets...)...)
		}
		...
		if len(es.curatorSecrets) > 0 {
			toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(rdata.CopySecrets(ElasticsearchNamespace, es.curatorSecrets...)...)...)
	...
	if es.supportsOIDC() {
		toCreate = append(toCreate, rdata.SecretsToRuntimeObjects(es.dexCfg.RequiredSecrets(ElasticsearchNamespace)...)...)
	}
...
func (es elasticsearchComponent) podTemplate() corev1.PodTemplateSpec {
	...
	podTemplate := corev1.PodTemplateSpec{
		...
		Spec: corev1.PodSpec{
			...
			ImagePullSecrets:   rdata.GetImagePullSecretReferenceList(es.pullSecrets),
