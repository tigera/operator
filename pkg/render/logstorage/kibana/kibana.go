...
var (
	EntityRule = networkpolicy.CreateEntityRule(Namespace, Name, KibanaPort)
	//SourceEntityRule = networkpolicy.CreateSourceEntityRule(Namespace, Name)
)
...
func (k *kibana) Objects() ([]client.Object, []client.Object) {
	...
	if k.cfg.UsePSP {
		if k.cfg.KibanaEnabled {
			toCreate = append(toCreate,
				k.kibanaClusterRoleBinding(),
				k.kibanaClusterRole(),
	...
	if k.cfg.KibanaEnabled {
		...
		toCreate = append(toCreate, k.kibanaAllowTigeraPolicy())
		toCreate = append(toCreate, networkpolicy.AllowTigeraDefaultDeny(Namespace))
		toCreate = append(toCreate, k.kibanaServiceAccount())
...
func (k *kibana) kibanaServiceAccount() *corev1.ServiceAccount {
...
func (k *kibana) kibanaCR() *kbv1.Kibana {
	...
	if k.cfg.BaseURL != "" {
		server["publicBaseUrl"] = fmt.Sprintf("%s/%s", k.cfg.BaseURL, KibanaBasePath)
	}
			...
			PodTemplate: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						KibanaTLSAnnotationHash: rmeta.SecretsAnnotationHash(k.kibanaSecrets...),
					},
					Labels: map[string]string{
						"name":    Name,
						"k8s-app": Name,
...
func (k *kibana) kibanaClusterRole() *rbacv1.ClusterRole {
...
func (k *kibana) kibanaClusterRoleBinding() *rbacv1.ClusterRoleBinding {
...
func (k *kibana) kibanaAllowTigeraPolicy() *v3.NetworkPolicy {
	egressRules := []v3.Rule{
		{
			Action:      v3.Allow,
