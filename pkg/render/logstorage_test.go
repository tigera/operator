	...
	Context("Standalone cluster type", func() {
		...
		Context("Updating LogStorage resource", func() {
			It("should create new NodeSet", func() {
				logStorage2 := &operator.LogStorage{
					ObjectMeta: metav1.ObjectMeta{
						Name: "tigera-secure",
					},
					Spec: operator.LogStorageSpec{
				...
				logStorage2.Spec.Nodes.ResourceRequirements = &corev1.ResourceRequirements{
					Limits: corev1.ResourceList{
						"cpu":    resource.MustParse("1"),
						"memory": resource.MustParse("150Mi"),
					},
				...
				createResources2, _ := component2.Objects()

				// Check updates to storage requirements are reflected
				Expect(createResources2[0].(*operatorv1.LogStorage).Spec.Nodes.ResourceRequirements).
					Should(Equal(logStorage2.Spec.Nodes.ResourceRequirements))
				
				newNodeName := createResources2[15].(*esv1.Elasticsearch).Spec.NodeSets[0].Name
