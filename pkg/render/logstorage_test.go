	...
	Context("Standalone cluster type", func() {
		...
		It("should not panic if an empty spec is provided", func() {
			...
			component := render.LogStorage(
				...
				}, nil, operatorv1.ProviderNone, nil, nil, nil, "cluster.local", nil, render.ElasticsearchLicenseTypeEnterpriseTrial)
		...
		Context("Initial creation", func() {
			...
			It("should render an elasticsearchComponent and delete the Elasticsearch and Kibana ExternalService", func() {
				...
				component := render.LogStorage(
					...
					[]*corev1.Secret{
						{ObjectMeta: metav1.ObjectMeta{Name: "tigera-pull-secret"}},
					}, nil, operatorv1.ProviderNone, nil,
					&corev1.Service{
		...
		It("Configures OIDC for Kibana when the OIDC configuration is provided", func() {
			...
			component := render.LogStorage(
				...
				[]*corev1.Secret{
					{ObjectMeta: metav1.ObjectMeta{Name: "tigera-pull-secret"}},
				}, nil, operatorv1.ProviderNone, nil, nil, nil, "cluster.local", dexCfg, render.ElasticsearchLicenseTypeEnterpriseTrial,
			)
		...
		It("should not configures OIDC for Kibana when elasticsearch basic license is used", func() {
			...
			component := render.LogStorage(
				...
				[]*corev1.Secret{
					{ObjectMeta: metav1.ObjectMeta{Name: "tigera-pull-secret"}},
				}, nil, operatorv1.ProviderNone, nil, nil, nil, "cluster.local", dexCfg, render.ElasticsearchLicenseTypeBasic)
