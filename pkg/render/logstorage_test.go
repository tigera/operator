...
var _ = Describe("Elasticsearch rendering tests", func() {
	Context("Standalone cluster type", func() {
		...
		var oidcConfigMap *corev1.ConfigMap
			...
			It("should render an elasticsearchComponent and delete the Elasticsearch and Kibana ExternalService", func() {
				...
				component := render.LogStorage(
					...
					&corev1.Service{
						...
						Spec:       corev1.ServiceSpec{Type: corev1.ServiceTypeExternalName},
					}, "cluster.local", false, nil, render.ElasticsearchLicenseTypeBasic, oidcConfigMap, oidcSecret)
		...
		It("Configures OIDC for Kibana when the OIDC configuration is provided", func() {
			...
			component := render.LogStorage(
				...
				[]*corev1.Secret{
					{ObjectMeta: metav1.ObjectMeta{Name: "tigera-pull-secret"}},
				}, operatorv1.ProviderNone, nil, nil, nil, "cluster.local", true, dexCfg, render.ElasticsearchLicenseTypeEnterpriseTrial, oidcConfigMap, oidcSecret,
			)
			...
			oidcUserConfigMap := rtest.GetResource(deleteResources, render.OIDCUsersConfigMapName, render.ElasticsearchNamespace, "", "v1", "ConfigMap")
			Expect(oidcUserConfigMap).ShouldNot(BeNil())

			oidcUserSecret := rtest.GetResource(deleteResources, render.OIDCUsersEsSecreteName, render.ElasticsearchNamespace, "", "v1", "Secret")
		...
		It("should not configures OIDC for Kibana when elasticsearch basic license is used", func() {
			...
			oidcUserConfigMap := rtest.GetResource(createResources, render.OIDCUsersConfigMapName, render.ElasticsearchNamespace, "", "v1", "ConfigMap")
			Expect(oidcUserConfigMap).ShouldNot(BeNil())

			oidcUserSecret := rtest.GetResource(createResources, render.OIDCUsersEsSecreteName, render.ElasticsearchNamespace, "", "v1", "Secret")
			Expect(oidcUserSecret).ShouldNot(BeNil())
		})
	})
		...
		Context("Initial creation", func() {
			It("creates Managed cluster logstorage components", func() {
				...
				component := render.LogStorage(
					...
					nil, nil, nil, "cluster.local", true, nil, render.ElasticsearchLicenseTypeEnterpriseTrial, nil, nil)
