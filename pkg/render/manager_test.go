...
var _ = Describe("Tigera Secure Manager rendering tests", func() {
	...
	It("should render all resources for a default configuration", func() {
		...
		Expect(voltron.VolumeMounts[1].Name).To(Equal(render.EsGatewayVolumeName))
		...
		Expect(deployment.Spec.Template.Spec.Volumes[1].Name).To(Equal(render.EsGatewayVolumeName))
		Expect(deployment.Spec.Template.Spec.Volumes[1].Secret.SecretName).To(Equal(render.EsGatewayKibanaPublicCertSecret))
		...
		Expect(deployment.Spec.Template.Spec.Volumes[3].Secret.SecretName).To(Equal(render.EsGatewayElasticPublicCertSecret))
	})
	...
	It("should render multicluster settings properly", func() {
		...
		Expect(deployment.Spec.Template.Spec.Volumes[6].Secret.SecretName).To(Equal(render.EsGatewayElasticPublicCertSecret))
