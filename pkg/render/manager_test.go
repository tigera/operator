...
var _ = Describe("Tigera Secure Manager rendering tests", func() {
	...
	It("should render the correct ClusterRole", func() {
		...
		Expect(clusterRole.Rules).To(ConsistOf([]rbacv1.PolicyRule{
			...
			// RBAC management UI: rendered on every non-multi-tenant cluster, since
			// the feature is activated at runtime from the rbac-ui-config ConfigMap.
			{
	...
	Context("RBAC management UI", func() {
		...
		// is the stable presence check for the RBAC management UI access.
		...
		// operator renders the access unconditionally.
