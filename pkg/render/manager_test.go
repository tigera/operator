...
var _ = Describe("Tigera Secure Manager rendering tests", func() {
	...
	Context("WAF management UI", func() {
		...
		// ui-apis reads WAF_UI_ENABLED at startup rather than watching the gate, so the
		// admin's value has to reach it through the pod spec.
		DescribeTable("projects the gate onto WAF_UI_ENABLED",
...
type renderConfig struct {
	...
	// wafManagementEnabled mirrors the admin's waf-ui-config value.
