...
var _ = Describe("monitor rendering tests", func() {
	...
	It("Should render Prometheus resource Specs correctly", func() {
		...
		Expect(prometheusOperatedHttpServiceManifest.Spec.Ports[0].Port).To(Equal(int32(common.PrometheusDefaultPort)))
		Expect(prometheusOperatedHttpServiceManifest.Spec.Ports[0].TargetPort.IntVal).To(Equal(int32(common.PrometheusDefaultPort)))
	})
})
