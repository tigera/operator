...
func (c *nodeComponent) vppVolumes() []v1.Volume {
	...
	if c.cfg.Installation.KubernetesProvider == operator.ProviderEKS {
...
func (c *nodeComponent) vppResources(isAgent bool) v1.ResourceRequirements {
	...
	if isAgent {
		...
		if c.cfg.Installation.CalicoNetwork.VPP.UplinkDriver == "dpdk" && c.cfg.Installation.KubernetesProvider == operator.ProviderEKS {
...
func (c *nodeComponent) nodeEnvVars() []v1.EnvVar {
	...
	if !bgpEnabled(c.cfg.Installation) {
		if c.cfg.Installation.CNI.Type == operator.PluginCalico {
			if c.cfg.Installation.CNI.IPAM.Type == operator.IPAMPluginHostLocal || vppDataplaneEnabled(c.cfg.Installation) {
