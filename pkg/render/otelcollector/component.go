...
func (c *component) networkPolicy() *v3.NetworkPolicy {
	...
	for _, exp := range c.cfg.OTelCollector.Exporters {
		if p := exporterPort(exp.Endpoint); p > 0 {
...
// exporterPort extracts the port number from an exporter endpoint string.
// Handles both bare host:port ("otlp.example.com:4317") and full URLs
// ("https://otlp.example.com:443"). Returns 0 if no port can be determined.
func exporterPort(endpoint string) int {
	// Try bare host:port first — url.Parse misinterprets the host as scheme.
	if _, portStr, err := net.SplitHostPort(endpoint); err == nil {
		if p, err := strconv.Atoi(portStr); err == nil && p > 0 && p <= 65535 {
			return p
		}
	}
	// Fall back to full URL parsing for endpoints with a scheme.
