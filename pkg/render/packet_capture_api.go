...
func (pc *packetCaptureApiComponent) Objects() ([]client.Object, []client.Object) {
	...
	if pc.cfg.ServerCertSecret.UseCertificateManagement() {
		...
		toDelete = append(toDelete, pc.cfg.ServerCertSecret.Secret(PacketCaptureNamespace))
	} else {
		objs = append(objs, pc.cfg.ServerCertSecret.Secret(PacketCaptureNamespace))
		toDelete = append(toDelete, CSRClusterRoleBinding(PacketCaptureServiceName, PacketCaptureNamespace))
	}
