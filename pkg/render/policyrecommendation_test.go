...
var _ = Describe("Policy recommendation rendering tests", func() {
	...
	It("should render a deployment with the MANAGED_CLUSTER_IS_CALICO set to true for a management cluster for calico tenant", func() {
		// Test for cluster connection type
		cfg.ManagedCluster = false
		cfg.ManagementCluster = true
		...
		Expect(deployment).NotTo(BeNil())
		Expect(deployment.Spec.Template.Spec.Containers).To(HaveLen(1))
		Expect(deployment.Spec.Template.Spec.Containers[0].Env).To(ContainElement(corev1.EnvVar{Name: "MANAGED_CLUSTER_IS_CALICO", Value: "true"}))
	})

	It("should render a deployment without the MANAGED_CLUSTER_IS_CALICO for a management cluster for enterprise tenant", func() {
		// Test for cluster connection type
		cfg.ManagedCluster = false
		cfg.ManagementCluster = true
		cfg.Tenant = &operatorv1.Tenant{
		...
		Expect(deployment.Spec.Template.Spec.Containers[0].Env).ToNot(ContainElement(WithTransform(
			func(envVar corev1.EnvVar) string { return envVar.Name }, Equal("MANAGED_CLUSTER_IS_CALICO"))))
	})

	It("should render a deployment without the MANAGED_CLUSTER_IS_CALICO for a management cluster without tenant info", func() {
		// Test for cluster connection type
		cfg.ManagedCluster = false
		cfg.ManagementCluster = true
		component := render.PolicyRecommendation(cfg)
