...
var _ = Describe("Rendering tests", func() {
	...
	It("should render all resources for a default configuration", func() {
		...
		// - 1 namespace
		// - 1 PriorityClass
		c, err := render.Calico(k8sServiceEp, instance, nil, nil, nil, typhaNodeTLS, nil, nil, operator.ProviderNone, nil, false)
	...
	It("should render all resources when variant is Tigera Secure and Management Cluster", func() {
		...
		c, err := render.Calico(k8sServiceEp, instance, &operator.ManagementCluster{}, nil, nil, typhaNodeTLS, nil, nil, operator.ProviderNone, nil, false)
		Expect(err).To(BeNil(), "Expected Calico to create successfully %s", err)
