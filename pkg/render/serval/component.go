...
// Package serval renders Serval: the single layer-7 entrypoint
...
const (
	...
	ServalTyphaClientKeyPairSecret = "serval-typha-client-certs"

	ServalServicePortName = common.ServalServicePortName
...
type Configuration struct {
	...
	// TyphaClientKeyPair is the client keypair Serval presents to the in-cluster
	// calico-typha. Its common name (typha-client) is one calico-typha accepts,
	// so Serval relays felix connections to it as an ordinary Typha client.
	TyphaClientKeyPair certificatemanagement.KeyPairInterface

	// K8sServiceEp is the Kubernetes API endpoint override (from the
	// kubernetes-services-endpoint ConfigMap). Serval's own client needs it to
	// reach the apiserver on clusters without kube-proxy (e.g. eBPF) or with a
	// custom apiserver address.
	K8sServiceEp k8sapi.ServiceEndpoint

	// Deleted renders the component for teardown: Objects returns serval's
...
// objectsForDeletion lists serval's objects by reference (no spec), for the
...
func (c *Component) objectsForDeletion() []client.Object {
	return []client.Object{
		...
		// The operator-namespace copies are the source of truth for these keypairs,
		// so they outlive the Deployment unless they go too.
		&corev1.Secret{TypeMeta: metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"}, ObjectMeta: metav1.ObjectMeta{Name: ServalKeyPairSecret, Namespace: common.OperatorNamespace()}},
		&corev1.Secret{TypeMeta: metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"}, ObjectMeta: metav1.ObjectMeta{Name: ServalTyphaClientKeyPairSecret, Namespace: common.OperatorNamespace()}},
		&corev1.Secret{TypeMeta: metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"}, ObjectMeta: metav1.ObjectMeta{Name: ServalKeyPairSecret, Namespace: ServalNamespace}},
		&corev1.Secret{TypeMeta: metav1.TypeMeta{Kind: "Secret", APIVersion: "v1"}, ObjectMeta: metav1.ObjectMeta{Name: ServalTyphaClientKeyPairSecret, Namespace: ServalNamespace}},
	}
}
...
func (c *Component) clusterRole() *rbacv1.ClusterRole {
	return &rbacv1.ClusterRole{
		...
		Rules: []rbacv1.PolicyRule{
			...
			{
				// Log ingestion and Typha tunnel requests are authorized
				// against the host's RBAC with SubjectAccessReviews.
...
// typhaEndpoint is the in-cluster calico-typha address Serval relays felix
// tunnels to.
func (c *Component) typhaEndpoint() string {
	return fmt.Sprintf("%s.%s.svc.%s:%d",
		render.TyphaServiceName, common.CalicoNamespace, c.cfg.ClusterDomain, render.TyphaPort)
}

// typhaRelayEnvVars configures the Typha client connection Serval relays felix
// tunnels onto: the in-cluster calico-typha address, the CA that verifies its
// server certificate, the required server CN, and Serval's own client keypair.
func (c *Component) typhaRelayEnvVars() []corev1.EnvVar {
	return []corev1.EnvVar{
		{Name: "SERVAL_TYPHA_ENDPOINT", Value: c.typhaEndpoint()},
		{Name: "SERVAL_TYPHA_SERVER_NAME", Value: render.TyphaCommonName},
		{Name: "SERVAL_TYPHA_CA_BUNDLE_PATH", Value: c.cfg.TrustedCertBundle.MountPath()},
		{Name: "SERVAL_TYPHA_CLIENT_CERT_PATH", Value: c.cfg.TyphaClientKeyPair.VolumeMountCertificateFilePath()},
		{Name: "SERVAL_TYPHA_CLIENT_KEY_PATH", Value: c.cfg.TyphaClientKeyPair.VolumeMountKeyFilePath()},
	}
}
...
func (c *Component) container() corev1.Container {
	...
	// The tunnel relays each felix connection to the in-cluster calico-typha.
	env = append(env, c.typhaRelayEnvVars()...)
	// KUBERNETES_SERVICE_HOST/PORT overrides so Serval's own client reaches the
	// apiserver on clusters without kube-proxy or with a custom apiserver address.
	env = append(env, c.cfg.K8sServiceEp.EnvVars()...)

	volumeMounts := []corev1.VolumeMount{
		c.cfg.ServerKeyPair.VolumeMount(c.SupportedOSType()),
		c.cfg.TyphaClientKeyPair.VolumeMount(c.SupportedOSType()),
	}
	...
	d := &appsv1.Deployment{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				c.cfg.TyphaClientKeyPair.HashAnnotationKey(): c.cfg.TyphaClientKeyPair.HashAnnotationValue(),
			},
		},
		Spec: appsv1.DeploymentSpec{
			// Replicas are owned by the HostEndpoint-count autoscaler: Serval's
			// in-process Typha fans out to the registered hosts, so it scales with
			// their number. The render deliberately leaves the count unset.
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Volumes: []corev1.Volume{
						...
						c.cfg.TyphaClientKeyPair.Volume(),
...
func (c *Component) networkPolicy() *v3.NetworkPolicy {
	...
	egressRules = append(egressRules,
		// Serval's own client (token review, impersonation) and the tunnel relay
		// reach the apiserver and the in-cluster Typha respectively.
		v3.Rule{
			Action:      v3.Allow,
			Protocol:    &networkpolicy.TCPProtocol,
		...
		v3.Rule{
			// Serval relays to calico-typha through its ClusterIP, so match the
			// Service (a plain endpoint selector does not match ClusterIP-DNAT'd
			// traffic). A service selector implies the Service's ports, so Ports
			// must not be set alongside it.
			Action:      v3.Allow,
			Protocol:    &networkpolicy.TCPProtocol,
			Destination: networkpolicy.CreateServiceSelectorEntityRule(common.CalicoNamespace, render.TyphaServiceName),
		},
		v3.Rule{
			Action:   v3.Allow,
			Protocol: &networkpolicy.TCPProtocol,
