...
type TigeraIstioResources struct {
	CRDs    []client.Object
	Base    []client.Object
...
func GetResources(namespace, releaseName string, baseOptions, istiodOptions, cniOptions,
	ztunnelOptions map[string]interface{}) (*TigeraIstioResources, error) {
	...
	res := &TigeraIstioResources{}
	rel, err := client.Run(baseChart, baseOptions)
	if err != nil {
		return nil, err
	}
	crds, res.Base, err = parseManifest(rel.Manifest)
	if err != nil {
		return nil, err
	}
	...
	crds, res.Istiod, err = parseManifest(rel.Manifest)
	if err != nil {
		return nil, err
	}
	res.CRDs = append(res.CRDs, crds...)
	...
	crds, res.CNI, err = parseManifest(rel.Manifest)
	if err != nil {
		return nil, err
	}
	res.CRDs = append(res.CRDs, crds...)
	...
	crds, res.ZTunnel, err = parseManifest(rel.Manifest)
	if err != nil {
		return nil, err
	}
	res.CRDs = append(res.CRDs, crds...)
...
func parseManifest(manifest string) (crds, objs []client.Object, err error) {
