...
"plugins": [
  {
	...
	Context("With calico-node-windows DaemonSet overrides", func() {
		...
		It("should handle calicoNodeWindowsDaemonSet overrides", func() {
			...
			// daemonset such as "k8s-app=calico-node-windows". The calico-node-windows
			// daemonset object produced by the render carries two labels: the
			// operator's host-networked marker (applied unconditionally because the
			// windows DaemonSet is hostNetwork) and the override-supplied
			// template-level label.
