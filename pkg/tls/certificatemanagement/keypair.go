...
func tlsSecretMetadata(certPEM []byte) (labels map[string]string, annotations map[string]string) {
	...
	// --- labels ---
	labels["operator.tigera.io/secret-type"] = "tls"

	signerLabel := signerLabelValue(cert.Issuer.CommonName)
	labels["operator.tigera.io/signer"] = signerLabel
...
// signerLabelValue extracts a Kubernetes-label-safe signer identifier from the
// issuer CN. It takes the portion before '@' (if any) and truncates to 63 chars.
